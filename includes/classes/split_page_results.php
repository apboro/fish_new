...
/* class constructor */
    function splitPageResults($query, $max_rows, $count_key = '*', $page_holder = 'page', $total = false) {
      global $_GET, $_POST;
      ...
      if (strpos($this->sql_query, 'distinct') || strpos($this->sql_query, 'group by')) {
        ...
        if (stripos($this->sql_query, 'SELECT SQL_CALC_FOUND_ROWS') !== false) {
            $offset = ($this->number_of_rows_per_page * ($this->current_page_number - 1));
            $sql_query = $this->sql_query . " limit " . max($offset, 0) . ", " . $this->number_of_rows_per_page;
            $sql_query = tep_db_query($sql_query);
            $count_query = tep_db_query('SELECT FOUND_ROWS() as total');
...
// BOM Mod:allow for a call when there are no rows to be displayed
      ...
      $parameters=preg_replace('|sort=products_sort_order|','',$parameters);
