...
define('EP_SPLIT_MAX_RECORDS', 3000);  // default, seems to work for most people.  Reduce if you hit timeouts
...
//
$custom_fields[TABLE_PRODUCTS] = array('products_id' => 'ID of product', 'products_quantity_order_min' => TEXT_EASYPOPULATE_LABEL_QUANTITY_MIN, 'products_quantity_order_units' => TEXT_EASYPOPULATE_LABEL_QUANTITY_UNITS, 'products_sort_order' => TEXT_EASYPOPULATE_LABEL_SORT, 'products_purchase_price' => 'Закупочная цена','supplier_id' => 'Поставщик'); // this line is used if you have no custom fields to import/export
...
$custom_fields[TABLE_PRODUCTS_DESCRIPTION] = array('products_barcode' => TEXT_EASYPOPULATE_LABEL_BARCODE);
//
...
foreach ($languages as $key => $lang) {
    ...
    $default_these[] = 'v_og_title_' . $lang['id'];
    $default_these[] = 'v_og_description_' . $lang['id'];
    $default_these[] = 'v_og_image_' . $lang['id'];
    $default_these[] = 'v_tk_title_' . $lang['id'];
    $default_these[] = 'v_supplier_' . $lang['id'];
    $default_these[] = 'v_tk_description_' . $lang['id'];
    $default_these[] = 'v_tk_image_' . $lang['id'];
    ...
    while ($row) {
        ...
        foreach ($languages as $key => $lang) {
            ...
            */
            ...
            $row['v_products_name_' . $lid] = $row['products_name'];
            $row['v_products_description_' . $lid] = $row['products_description'];
            $row['v_products_url_' . $lid] = $row['products_url'];
            $row['v_og_title_' . $lid] = $row['og_title'];
            $row['v_og_description_' . $lid] = $row['og_description'];
            $row['v_og_image_' . $lid] = $row['og_image'];
            $row['v_tk_title_' . $lid] = $row['tk_title'];
            $row['v_tk_description_' . $lid] = $row['tk_description'];
            $row['v_supplier_' . $lid] = $row['supplier_name'];
            $row['v_tk_image_' . $lid] = $row['tk_image'];
            foreach ($custom_fields[TABLE_PRODUCTS_DESCRIPTION] as $key => $name) {
                $row['v_' . $key . '_' . $lid] = $row[$key];
            }
            // froogle advanced format needs the quotes around the name and desc
            $row['v_froogle_products_name_' . $lid] = '"' . strip_tags(str_replace('"', '""', $row['products_name'])) . '"';
            $row['v_froogle_products_description_' . $lid] = '"' . strip_tags(str_replace('"', '""', $row['products_description'])) . '"';

            // support for Linda's Header Controller 2.0 here
            if (isset($filelayout['v_products_head_title_tag_' . $lid])) {
                $row['v_products_head_title_tag_' . $lid] = $row['products_head_title_tag'];
                $row['v_products_head_desc_tag_' . $lid] = $row['products_head_desc_tag'];
                $row['v_products_head_keywords_tag_' . $lid] = $row['products_head_keywords_tag'];
            }
    ...
    <table border="0" width="100%" cellspacing="2" cellpadding="2">
        <tr>
                    ...
                    <tr>
                        <td>
                            ...
                            <form name="clear_form"
                                  action="easypopulate.php<?php if (defined('SID') && tep_not_null(SID)) {
                                      echo '?' . tep_session_name() . '=' . tep_session_id();
                                  } ?>" method="post" id="clear_form">
                                <input type="submit" name="clear_button" value="<?php echo TEXT_EASYPOPULATE_CLEAR; ?>"
                            ...
                            if (EP_EXCEL_SAFE_OUTPUT == true) {
                                ...
                                if (EP_EXCEL_SAFE_OUTPUT_ALT_PARCE == false) {
                                    ...
                                    while ($line = fgetcsv($fp, 32768, $ep_separator))   // read new line (max 32K bytes)
                                    {
                                        ...
                                        /*---by iHolder transfer to UTF-8--*/
                                        if ($_POST['import_charset'] == 'cp1251') {
                                            $line = CP1251toUTF8($line);
                                        }
                                        /*---by iHolder transfer to UTF-8--*/
                            ...
                            foreach ($readed as $tkey => $readed_row) {
                                process_row($readed_row, $filelayout, $filelayout_count, $default_these, $ep_separator, $languages, $custom_fields,$_POST['id']);
                            }
                            // isn't working in PHP 5
                            // array_walk($readed, $filelayout, $filelayout_count, $default_these, 'process_row');
                            ?>
                            </p>
                            <form name="clear_form2"
                                  action="easypopulate.php<?php if (defined('SID') && tep_not_null(SID)) {
                                      echo '?' . tep_session_name() . '=' . tep_session_id();
                                  } ?>" method="post" id="clear_form2">
                                <input type="submit" name="clear_button2" value="<?php echo TEXT_EASYPOPULATE_CLEAR; ?>"
                                       style="padding: 0px"/></form>
                            <?php
                            ...
                            //*******************************
                                ...
                                <form name="clear_form2"
                                    ...
                                    <input type="submit" name="clear_button2"
                                           value="<?php echo TEXT_EASYPOPULATE_CLEAR; ?>" style="padding: 0px"/></form>
                                <?php
                                echo "<p class=smallText>";
                                // move the file to where we can work with it
                                $file = tep_get_uploaded_file('usrfl');
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                                <tr>
                                    <td class="main" width="100%"><span style="background-color:#FFFFCC; width:100%;">&nbsp; &nbsp;<?php echo TEXT_EASYPOPULATE_PLEASE; ?>
                                            <a href="<?php echo tep_href_link(FILENAME_BACKUP); ?>"><u><?php echo TEXT_EASYPOPULATE_BACKUP_TEXT; ?></u></a><?php echo TEXT_EASYPOPULATE_BACKUP_TEXT1; ?></span>&nbsp;
                                        <?php
                                        // Quick Backup Button
                                        if (EP_QUICK_BACKUP == true) {
                                            ?>
                                            <form name="skb" action="backup.php" method="post" id="quick_backup">
                                                <input type="submit" name="quick_backup_button"
                                                       value="<?php echo TEXT_EASYPOPULATE_BACKUP_BUTTON; ?>"
                                                       style="padding: 0px"/></form>
                                            <?php
                                        }
                                        ?>
                                        ...
                                        <div id="tabs">
                                            ...
                                            <ul>
                                                ...
                                                <li>
                                                    <a href="#import-temp"><?php echo EASYPOPULATE_TAB_IMPORT_TEMP; ?></a>
                                                </li>
                                                <li><a href="#split"><?php echo EASYPOPULATE_TAB_SPLIT; ?></a></li>
                                                <li><a href="#export"><?php echo EASYPOPULATE_TAB_EXPORT; ?></a></li>
                                                <li>
                                                    <a href="#quick-links"><?php echo EASYPOPULATE_TAB_QUICK_LINKS; ?></a>
                                                </li>
                                            ...
                                            <div id="import">
                                                ...
                                                <p>
                                                    ...
                                                    <input type="submit" name="buttoninsert"
                                                           value="<?php echo EASY_INSERT; ?>">
                                                   <label><input name="id" value="article" checked="" type="radio"/>Артикул</label>
                                                   <label><input name="id" value="id" type="radio"/>Id</label>
                                                    <br/>
                                                </p>
                                                </form>
                                            ...
                                            <div id="split">
                                                ...
                                                <p>
                                                    ...
                                                    <input name="usrfl" type="file" size="50">
                                                    <input type="submit" name="buttonsplit"
                                                           value="<?php echo EASYPOPULATE_BUTTON_SPLIT; ?>">
                                                    <br/>
                                                </p>
                                                </form>
                                            ...
                                            <div id="export">
                                                ...
                                                <table border="0" cellpadding="0" cellspacing="0"
                                                    ...
                                                    <?php echo tep_draw_form('custom', 'easypopulate.php', ((defined('SID') && tep_not_null(SID)) ? tep_session_name() . '=' . tep_session_id() : ''), 'get', 'id="custom"'); ?><?php if (defined('SID') && tep_not_null(SID)) {
			...
			<option value="utf8" size="5">utf8</option>
                                                            ...
                                                            $cells[0][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_supplier', 'show', (!empty($_GET['epcust_supplier']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_SUPPLIER . '</td></tr></table>');
                                                            ...
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_og_title', 'show', (!empty($_GET['epcust_og_title']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_OG_TITLE . '</td></tr></table>');
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_og_description', 'show', (!empty($_GET['epcust_og_description']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_OG_DESCRIPTION . '</td></tr></table>');
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_og_image', 'show', (!empty($_GET['epcust_og_image']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_OG_IMAGE . '</td></tr></table>');
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_tk_title', 'show', (!empty($_GET['epcust_tk_title']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_TK_TITLE . '</td></tr></table>');
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_tk_description', 'show', (!empty($_GET['epcust_tk_description']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_TK_DESCRIPTION . '</td></tr></table>');
                                                            $cells[3][] = array('text' => '<table border="0" cellpadding="0" cellspacing="0"><tr><td class="smallText">' . tep_draw_checkbox_field('epcust_tk_image', 'show', (!empty($_GET['epcust_tk_image']) ? true : false)) . '</td><td class="smallText"> ' . TEXT_EASYPOPULATE_LABEL_TK_IMAGE . '</td></tr></table>');
                                                    ...
                                                    <tr>
                                                        <td width="50%" valign="top">
                                                            <p style="margin-top: 8px;">
                                                                <b><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_1; ?></b><br/>
                                                                <font size="-2"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_2; ?></font>
                                                            </p>
                                                            <p>
                                                                <!-- Download file links -  Add your custom fields here -->
                                                                <a href="easypopulate.php?download=stream&dltype=full<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_3; ?><?php if (EP_SPPC_SUPPORT == true) {
                                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_4;
                                                                    } ?></b> <?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_5; ?></a><br/>
                                                                <?php if (EP_EXTRA_FIELDS_SUPPORT == true) { ?>
                                                                    <a href="easypopulate.php?download=stream&dltype=extra_fields<?php if (defined('SID') && tep_not_null(SID)) {
                                                                        echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                    } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_7; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_5; ?></a>
                                                                    <br/>
                                                                <?php } ?>
                                                                <a href="easypopulate.php?download=stream&dltype=priceqty<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_8; ?><?php if (EP_SPPC_SUPPORT == true) {
                                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_4;
                                                                    } ?></b> <?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_5; ?></a><br/>
                                                                <a href="easypopulate.php?download=stream&dltype=category<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_9; ?></b> <?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_5; ?></a><br/>
                                                                <a href="easypopulate.php?download=stream&dltype=froogle<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_10; ?></b> <?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_12; ?></a><br/>
                                                                <!-- VJ product attributes begin //-->
                                                                <?php if (EP_PRODUCTS_WITH_ATTRIBUTES == true) { ?>
                                                                    <a href="easypopulate.php?download=stream&dltype=attrib<?php if (defined('SID') && tep_not_null(SID)) {
                                                                        echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                    } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_11; ?></b> <?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_12; ?></a>
                                                                    <br/>
                                                                <?php } ?>
                                                                <!-- VJ product attributes end //-->
                                                            </p><br/>
                                                        ...
                                                        <td width="50%" valign="top">
                                                            <p style="margin-top: 8px;">
                                                                <b><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_13; ?></b><br/>
                                                                <font size="-2"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_14; ?></font>
                                                            </p>
                                                            <p>
                                                                <a href="easypopulate.php?download=tempfile&dltype=full<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_15; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_6; ?></a><br/>
                                                                <a href="easypopulate.php?download=tempfile&dltype=priceqty<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_16; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_6; ?></a><br/>
                                                                <a href="easypopulate.php?download=tempfile&dltype=category<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_17; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_6; ?></a><br/>
                                                                <a href="easypopulate.php?download=tempfile&dltype=froogle<?php if (defined('SID') && tep_not_null(SID)) {
                                                                    echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_18; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_6; ?></a><br/>
                                                                <!-- VJ product attributes begin //-->
                                                                <?php if (EP_PRODUCTS_WITH_ATTRIBUTES == true) { ?>
                                                                    <a href="easypopulate.php?download=tempfile&dltype=attrib<?php if (defined('SID') && tep_not_null(SID)) {
                                                                        echo '&' . tep_session_name() . '=' . tep_session_id();
                                                                    } ?>"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_19; ?><?php echo((EP_EXCEL_SAFE_OUTPUT == true) ? ".csv" : ".txt"); ?><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_6; ?></a>
                                                                    <br/>
                                                                <?php } ?>
                                                                <!-- VJ product attributes end //-->
                                                            </p><br/>
                                            ...
                                            <?php if (EP_SHOW_EP_SETTINGS == true) { ?>
                                                ...
                                                <div id="info">
                                                    ...
                                                    <?php if (is_writable(EP_TEMP_DIRECTORY)) { ?><font
                                                            color="#009900"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_21; ?></font><?php } else { ?>
                                                        <font color="#FF0000"><?php echo TEXT_EASYPOPULATE_QUICK_LINKS_22; ?></font><?php } ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_23; ?> <?php if (ini_get('magic_quotes_runtime') == 1) {
                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_42;
                                                    } else {
                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_43;
                                                    } ?><br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_24; ?> <?php if (ini_get(register_globals)) {
                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_42;
                                                    } else {
                                                        echo TEXT_EASYPOPULATE_QUICK_LINKS_43;
                                                    } ?><br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_25; ?> <?php echo EP_SPLIT_MAX_RECORDS; ?> <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_44; ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_26; ?> <?php echo EP_MODEL_NUMBER_SIZE; ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_27; ?> <?php echo(EP_PRICE_WITH_TAX ? 'true' : 'false'); ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_28; ?> <?php echo EP_PRECISION; ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_29; ?> <?php echo(EP_REPLACE_QUOTES ? 'true' : 'false'); ?>
                                                    <br/>
                                                    ...
                                                    ?><br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_37; ?> <?php echo(EP_EXCEL_SAFE_OUTPUT ? 'true' : 'false'); ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_38; ?> <?php echo(EP_PRESERVE_TABS_CR_LF ? 'true' : 'false'); ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_39; ?> <?php echo EP_MAX_CATEGORIES; ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_40; ?> <?php echo(EP_PRODUCTS_WITH_ATTRIBUTES ? 'true' : 'false'); ?>
                                                    <br/>
                                                    <?php echo TEXT_EASYPOPULATE_QUICK_LINKS_41; ?> <?php echo(EP_FROOGLE_SEF_URLS ? 'true' : 'false'); ?>
                                                    <br/>
                                                    <br/>
    ...
    switch ($dltype) {
        ...
        case 'full':
            ...
            foreach ($languages as $key => $lang) {
                ...
                $filelayout['v_og_title_' . $lang['id']] = $iii++;
                $filelayout['v_og_description_' . $lang['id']] = $iii++;
                $filelayout['v_og_image_' . $lang['id']] = $iii++;
                $filelayout['v_tk_title_' . $lang['id']] = $iii++;
                $filelayout['v_tk_description_' . $lang['id']] = $iii++;
                $filelayout['v_supplier_' . $lang['id']] = $iii++;
                $filelayout['v_tk_image_' . $lang['id']] = $iii++;
                foreach ($custom_fields[TABLE_PRODUCTS_DESCRIPTION] as $key => $name) {
            ...
            p.supplier_id as v_supplier_id,
            ...
            FROM
              ".TABLE_PRODUCTS." as p
              INNER JOIN ".TABLE_PRODUCTS_DESCRIPTION." as pd ON p.products_id = pd.products_id
              INNER JOIN ".TABLE_PRODUCTS_TO_CATEGORIES." as ptoc ON ptoc.products_id = p.products_id
              INNER JOIN ".TABLE_CATEGORIES." as subc ON ptoc.categories_id = subc.categories_id
              LEFT JOIN ".TABLE_SUPPLIER." as s on s.supplier_id = p.supplier_id
            WHERE 1
            " . $sql_filter;
        ...
        case 'custom':
            ...
            foreach ($languages as $key => $lang) {
                ...
                if (!empty($_GET['epcust_supplier'])) {
                    $filelayout['v_supplier_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_og_title'])) {
                    $filelayout['v_og_title_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_og_description'])) {
                    $filelayout['v_og_description_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_og_image'])) {
                    $filelayout['v_og_image_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_tk_title'])) {
                    $filelayout['v_tk_title_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_tk_description'])) {
                    $filelayout['v_tk_description_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_supplier'])) {
                    $filelayout['v_supplier_' . $lang['id']] = $iii++;
                }
                if (!empty($_GET['epcust_tk_image'])) {
                    $filelayout['v_tk_image_' . $lang['id']] = $iii++;
                }
                foreach ($custom_fields[TABLE_PRODUCTS_DESCRIPTION] as $key => $name) {
...
///////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////
function process_row($item1, $filelayout, $filelayout_count, $default_these, $ep_separator, $languages, $custom_fields,$id = 'article')
{
    ...
    if (isset($items[$filelayout['v_products_extra_fields_id']])) {
        ...
        if($id == 'id'){
            $sql = "SELECT p.products_id as v_products_id FROM " . TABLE_PRODUCTS . " as p WHERE p.products_id = '" . $v_products_id . "'";
        }else {
        ...
        // Get the ID
        if($id == 'id'){
            $sql = "SELECT p.products_id as v_products_id    FROM " . TABLE_PRODUCTS . " as p WHERE p.products_model = '" . $items[$filelayout['v_products_id']] . "'";
        }else {
        ...
        if($id == 'id'){
            $sql_add = "p.products_id = '" . $items[$filelayout['v_products_id']] . "'";
        }else {
            $sql_add = "p.products_model = '" . $items[$filelayout['v_products_model']] . "'";
        }
        ...
        if($id == 'id'){
            $v_products_model =  $row['v_products_model'];
        }
        ...
        if ($process_product == true) {
            ...
            while ($row) {
                ...
                ";
                    ...
                    $row['v_og_title_' . $lang['id']] = $row2['og_title'];
                    $row['v_og_description_' . $lang['id']] = $row2['og_description'];
                    $row['v_og_image_' . $lang['id']] = $row2['og_image'];
                    $row['v_tk_title_' . $lang['id']] = $row2['tk_title'];
                    $row['v_tk_description_' . $lang['id']] = $row2['tk_description'];
                    $row['v_supplier_' . $lang['id']] = $row2['supplier_name'];
                    $row['v_tk_image_' . $lang['id']] = $row2['tk_image'];
                    foreach ($custom_fields[TABLE_PRODUCTS_DESCRIPTION] as $key => $name) {
            ...
            if ($v_products_model != "" || $id == 'id') {
                ...
                if($id == 'id'){
                    $result = tep_db_query("SELECT products_id FROM " . TABLE_PRODUCTS . " WHERE (products_id = '" . $items[$filelayout['v_products_id']] . "')");
                }else {
                    $result = tep_db_query("SELECT products_id FROM " . TABLE_PRODUCTS . " WHERE (products_model = '" . $v_products_model . "')");
                }
                // First we check to see if this is a product in the current db.
                if (tep_db_num_rows($result) == 0 &&  $id !== 'id') {
                    ...
                    while ($row = tep_db_fetch_array($result)) {
                        ...
                        $id_query = '';
                        if($id === 'id'){
                            $id_query = 'products_model="'.$v_products_model.'",';
                        }
