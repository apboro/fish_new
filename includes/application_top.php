...
require_once(DIR_WS_FUNCTIONS.'mobile_func.php');
require_once(DIR_WS_FUNCTIONS.'pregeneral.php');
...
/*fix problem with empty cPath*/
if (isset($_GET['cPath'])&&(strlen($_GET['cPath'])==0)){unset($_GET['cPath'],$_REQUEST['cPath']);}
/*fix problem with empty cPath*/
...
$PHP_SELF = $SCRIPT_NAME=usu5_base_filename();
...
require(DIR_WS_FUNCTIONS . 'osc23.php');
...
if (function_exists('session_set_cookie_params')) {
    session_set_cookie_params(2838240000, $cookie_path, $cookie_domain,ENABLE_SSL,true);
...
$detect_category_link=CategoryLink();
...
if (!IS_MOBILE==1){
    ...
    require(MOBILE_PATH.'/'.DIR_WS_LANGUAGES . $language . '.php');
}
...
//added for xsell_cart
        ...
        */
        ...
        // customer adds a product from the products page
        case 'add_product' :
//      var_dump($_REQUEST);var_dump($_POST);var_dump($_GET);
//      var_dump($_SESSION);exit;
            ...
            if ($_REQUEST['buytype']<>'ajax'){
                if ( strlen($error_cart_msg)==0 ) {
                    tep_redirect(tep_href_link($goto, tep_get_all_get_params($parameters), 'NONSSL'));
                }
            }else{
                /*        $ToDo=array('type'=>'error','message'=>$error_cart_msg);
                        echo json_encode($ToDo); */
                if ($error_add){echo $error_cart_msg;}
                exit;
            }
            break;
...
if (isset($_GET['products_id'])) {
    ...
    if (tep_db_num_rows($model_query)) {
        ...
        if(defined('BREADCRUMB') && BREADCRUMB) {
...
if (isset($tPath_array)) {
    for ($i=0, $n=sizeof($tPath_array); $i<$n; $i++) {
        ...
        if (tep_db_num_rows($topics_query) > 0) {
            ...
            $breadcrumb->add($topics['topics_name'],tep_href_link(FILENAME_ARTICLES,'tPath='.(int)$tPath_array[$i]));
        } else {
            break;
        }
    }
...
if (isset($_GET['articles_id'])) {
    ...
    if (tep_db_num_rows($article_query)) {
        ...
        $breadcrumb->add($article['articles_name'],tep_href_link(FILENAME_ARTICLE_INFO,'articles_id='. (int)$_GET['articles_id']));
        /*
              if (isset($_GET['authors_id'])) {
                $breadcrumb->add($article['articles_name'],tep_href_link(FILENAME_ARTICLES_INFO,'articles_id='. (int)$_GET['articles_id']));
              } else {
...
/*---iHolder redirects*/
if (isset($_GET['cPath'])){
    if( (int)$_GET['cPath']==1239){tep_redirect(FILENAME_DISCOUNT);}
}
/*---iHolder redirects*/

require_once DIR_FS_CATALOG.'/seo-regions/regions.php';
global $regions;
$cache_pages = array('discount.php','index.php','product_info.php', 'information.php', 'price.php', 'products_new.php', 'featured_products.php','products_filter.php');
if (!tep_session_is_registered('customer_id') && ENABLE_PAGE_CACHE == 'true') {
    ...
    if (IS_MOBILE==0){
        ...
        require(MOBILE_PATH . '/includes/languages/'.$language. '/modules/boxes/bm_shopping_cart.php');
        require(MOBILE_PATH . '/includes/modules/boxes/' . 'bm_shopping_cart.php');
        $bmsc=new bm_shopping_cart();
        echo $bmsc->Build();
    }
    ...
    /*	foreach ($cache_pages as $index => $page){
            echo $index."<br>";
            echo "{".strpos($_SERVER['PHP_SELF'], $page)."}<br>";
            echo $page."<hr><br>";

    //		if (strpos($_SERVER['PHP_SELF'], $page) ){
    //		if (trim($_SERVER['PHP_SELF']==trim($page)) ){
    ...
    if (in_array($PHP_SELF,$cache_pages)){
        include(DIR_WS_CLASSES . 'page_cache.php');
        $page_cache = new page_cache($cart_cache);
        # $page_cache->cache_this_page(60);
        $page_cache->cache_this_page();
        $regions->usedCache(true);
    }
} # End if
if (IS_MOBILE==1 && !defined('AMP')){
//$SCRIPT_NAME=substr($_SERVER['SCRIPT_NAME'],1);
    $skip=in_array($SCRIPT_NAME,$script_skip);
    if (!$skip){
        require (DIR_WS_CLASSES.'osc_template.php');
        $oscTemplate = new oscTemplate();
        if (file_exists(MOBILE_PATH.'/'.$SCRIPT_NAME)){
            chdir(MOBILE_PATH);
            if (file_exists($SCRIPT_NAME)){include($SCRIPT_NAME);}
            exit;
        }else{
            //----try to assemble based on VAM edition
            //BuildMobileContent($SCRIPT_NAME);
            exit;
        }
    }
}?>
