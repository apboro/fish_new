...
?>
<div class="contentContainer" id="account_notifications">
    ...
    <div class="review_box">
    	<div class="content_box">
        	<span class="title"><?php echo MY_NOTIFICATIONS_TITLE; ?></span>
            <div class="contentText">
    			<?php echo MY_NOTIFICATIONS_DESCRIPTION; ?>
  			</div>
        </div>
        <div class="content_box">
        	<span class="title"><?php echo GLOBAL_NOTIFICATIONS_TITLE; ?></span>
            <div class="contentText">
                	<?php echo tep_draw_checkbox_field('product_global', '1', (($global['global_product_notifications'] == '1') ? true : false), 
						'onclick="checkBox(\'product_global\')"'); ?></td>
                    <?php echo GLOBAL_NOTIFICATIONS_TITLE; ?>, <?php echo GLOBAL_NOTIFICATIONS_DESCRIPTION; ?></td>
            </div>
        </div>
        <?php
		...
		?>
        <div class="content_box">
        	<span class="title"><?php echo NOTIFICATIONS_TITLE; ?></span>
             <div class="contentText">
			<?php
			...
			<?php
      			$counter = 0;
      			$products_query = tep_db_query("select pd.products_id, pd.products_name from " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_NOTIFICATIONS . " pn where 		
					pn.customers_id = '" . (int)$customer_id . "' and pn.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' order by pd.products_name");
      			while ($products = tep_db_fetch_array($products_query)) {
			?>
      		<tr>
        		<td width="30"><?php echo tep_draw_checkbox_field('products[' . $counter . ']', $products['products_id'], true, 
				'onclick="checkBox(\'products[' . $counter . ']\')"'); ?></td>
