<?php
if (isset($_GET['manufacturers_id'])){return;}
/*
?>
<!-- browse_categories !-->
<?php
...
function get_price_range($browse_category_id){
global $currencies;

$mm=GetMinMaxFromCat($browse_category_id );
if (is_array($mm)){$minmax_price=$currencies->format($mm['min']).'-'.$currencies->format($mm['max']);}
    else{$minmax_price='';}
return $minmax_price;
}
$minmax_price = get_price_range($browse_category_id);
 ...
 if($arr_current_categories_query) {
     $sub_category_count = tep_db_num_rows($arr_current_categories_query);
 }else{
     $sub_category_count = 0;
 }
  if (( $sub_category_count > 0) and (BRWCAT_ICON_MODE != 'off')) {
  ...
  // BOF Enable - Disable Categories Contribution--------------------------------------
    ...
    $content_box_contents[][] = array('align'  => 'center',
                                      'params' => 'class="smallText top-valign" colspan=2"',
   ...
   // $priceRanges = GetMinMaxFromCats($category_ids);
    foreach ($list_categories as $current_categories) {
      $cPath_cur = $cPaths_cur[$current_categories['categories_id']];
      ...
      if (BRWCAT_SUBCAT_MODE != 'off') {
        ...
        $sub_query = "select c.categories_id, cd.categories_name, c.categories_image
                        from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd, products p
      ...
      if (BRWCAT_ICON_MODE == 'text') {
        $content_box_contents[$row][$col] = array('align'  => 'center',
                                                  'params' => 'class="smallText top-valign" style="width:' . $table_cell_width . ';"',
        ...
        $content_box_contents[$row][$col] = array('align'  => 'center',
                                                  ...
                                                  'text'   => '<a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '">' .
                                                              tep_image(STATIC_DOMAIN.DIR_WS_IMAGES . $current_categories['categories_image'],
	...
	if ($c2c_array[$current_categories['categories_id']]>0){
        $content_box_contents[$row][$col] = array('align'  => 'center',
                                                  'params' => 'class="smallText top-valign" style="width:' . $table_cell_width . ';"',
                                                  'text'   => '<div class="tablodiv"><figure class="ramka"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '">' .
                                                              tep_image(STATIC_DOMAIN.DIR_WS_IMAGES . $current_categories['categories_image'],
                                                                        $current_category_name,
                                                                        SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT) .
                                                              '<align="center"></a><br><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '"><figcaption><b>' .
                                                              $current_category_name . '</b></figcaption></a>
                                                              <br> ' .
                                                              get_price_range($current_categories['categories_id'] ) . '</align></figure></div> ',
                                                              'content'=>true);
         }else{
            ...
            $content_box_contents[$row][$col] = array('align' => 'center',
                ...
                'text' => '<div class="tablodiv"><figure class="ramka"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '">' .
                    tep_image(STATIC_DOMAIN . DIR_WS_IMAGES . $current_categories['categories_image'],
                        $current_category_name,
                        SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT) .
                    '<align="center"></a><br><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '"><b>' .
                    $current_category_name . '</b></a>
                                                              <br> ' .
                    tep_image(STATIC_DOMAIN . DIR_WS_IMAGES . 'nt_nw_tm_cat.jpg') .
//                                                              get_price_range($current_categories['categories_id'] ) .
                    '</align></figure></div> ',
                'hide' => true, 'content' => true);
        }else{
            $content_box_contents[$row][$col] = array('align' => 'center',
                'params' => '',
                'text' => '',
                'hide' => true, 'content' => false);
        }
                                                             }
      ...
      if ($arr_sub_category_mode[0] == 'right') {
        $content_box_contents[$row][$col + 1] = array('align'  => 'left',
                                                      'params' => 'class="smallText" valign="' . $arr_sub_category_mode[1] . '" style="width:' . $table_cell_width . ';"',
...
/*-iHolder --resort results hidden to bottom---*/
$cat_visible=array();
$cat_hidden=array();
$exist_hidden=false;
for($i=1;$i<=$row;$i=$i+2){
    for($j=0;$j<BRWCAT_ICONS_PER_ROW;$j++){
	if (isset($content_box_contents[$i][$j])){
	if (isset($content_box_contents[$i][$j]['hide'])){
	    $exist_hidden=true;
	    $cat_hidden[]=$content_box_contents[$i][$j];
	    }else{
	    $cat_visible[]=$content_box_contents[$i][$j];
	    }
	    }
	}
    }
...
for($i=1;$i<=$row;$i=$i+2){
    for($j=0;$j<BRWCAT_ICONS_PER_ROW;$j++){
    if (sizeof($cat_visible)>0){
        $content_box_contents[$i][$j]=array_shift($cat_visible);
	}else{
        $content_box_contents[$i][$j]=array_shift($cat_hidden);
    }
    }
}
}
/*---resort results hidden to bottom---*/
//echo '<pre>';var_dump($content_box_contents);echo '</pre>';
//exit;
...
<div class="SA_CLB" onclick='SwitchCLB()'><i class="fa fa-sort" aria-hidden="true"></i><span> Показать все <?php echo $category['categories_heading_title']; ?></span></div>
<div class="CLB">
<script type="text/javascript">
...
<?} 
 $content_h_box_contents = array();
    $content_h_box_contents[] = array('align' => 'left',
                                    'text'  => $browse_category_heading);
    new infoboxHeading($content_h_box_contents, false, false);
