...
class Usu_Main
{
     ...
     * @return void
     */
    public $categories_cache = array();
    private $vars = array(); // end constructor
    ...
    {
        $c_cache = tep_db_query('select categories_id as id,categories_url as url from categories_description
		    where categories_url is not null or length(categories_url)>0');
        if ($c_cache !== false) {
            while ($r_cache = tep_db_fetch_array($c_cache)) {
                if (!empty($r_cache['url'])) {
                    $this->categories_cache[$r_cache['id']] = $r_cache['url'];
                }
            }
        }
        $this->setVar('categories_cache', $this->categories_cache);
    ...
    public function hrefLinks($page, $parameters, $page_ids, $connection, $add_session_id, $search_engine_safe)
    {
        ...
        if (false === ($url = $this->vars['page_modules'][$page_module_name]->buildLinks($page, $parameters, $page_ids, $add_session_id, $connection))) {
    ...
    public function hrefLink($page, $parameters, $connection, $add_session_id, $search_engine_safe)
    {
        ...
        if (!array_key_exists('initiated', $this->vars) || (false === $this->vars['initiated']) // USU5 not yet fully initiated
            ...
            || !array_key_exists($page_module_name, $this->vars['page_modules'])
        ) { // No module loaded for this page
            return osc_href_link($page, $parameters, $connection, $add_session_id, $search_engine_safe);
        }
