...
abstract class BarcodeGenerator
{
    ...
    protected function getBarcodeDataS($code, $type)
    {
        switch (strtoupper($type)) {
...
class BarcodeGeneratorHTML extends BarcodeGenerator
{
    ...
    {
	$code=preg_replace('|[^\d^\w]|','',$code);
	if (strlen($code)==0){return '';}
        $barcodeData = $this->getBarcodeData($code,self::TYPE_CODE_128);
...
public function getBarcodePNG($code,$widthFactor = 2, $totalHeight = 30, $color = array(0, 0, 0))
    {
	$code=preg_replace('|[^\d^\w]|','',$code);
