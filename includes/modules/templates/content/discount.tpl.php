...
$listing_sql='select distinct p.products_image, pd.products_name,p.products_quantity,
    if (p.products_quantity>0,1,0) as pq , p.products_id, 
    ...
    products p left join manufacturers_info mi on (p.manufacturers_id = mi.manufacturers_id and mi.languages_id = "1") 
    left join specials s on p.products_id = s.products_id, 
    products_to_categories p2c 
    where p.products_status = "1" and p.products_id = p2c.products_id 
    and pd.products_id = p2c.products_id 
    and pd.language_id = "1" and p.products_quantity>0 ';
if (sizeof($dproducts)>0){
...
<table width="100%" cellspacing="0" cellpadding="4" border="0" class="infoBoxContents">
  ...
  <tr>
  <td align="center">
   <a title="Сбросить" href="<?php echo tep_href_link(FILENAME_DEFAULT.'?cPath='.$current_category_id.(isset($_REQUEST['vls'])?'&vls='.(int)$_REQUEST['vls']:''));?>">
