    ...
    function execute() {
      ...
      if (!isset($HTTP_GET_VARS['products_id'])) {
        ...
        if (tep_db_num_rows($best_sellers_query) >= MIN_DISPLAY_BESTSELLERS) {
          $bestsellers_list = '<ul class="mj-bullet21">';
          while ($best_sellers = tep_db_fetch_array($best_sellers_query)) {
            $bestsellers_list .= '<li><i class="fa fa-angle-right fa-lg"><a class="bstsaller_text" href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $best_sellers['products_id']) . '">' . $best_sellers['products_name'] . '</a></i></li>';
          }
          $bestsellers_list .= '</ul>';

          $data = '<div class="ui-widget infoBoxContainer mj-bestsellers">' .
