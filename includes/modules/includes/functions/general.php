...
function GetMinMaxFromCat($catId){//----by iHolder get min max
    $c2c_array=array();
	...
	$cids[]=$catId;
		...
		if (is_array($c2c_array[$cid])){
		    foreach($c2c_array[$cid] as $key=>$value){$cids[]=$value;}
		    }
		}while (sizeof($cids)>0);
...
function GetDeductionMap(){//----by iHolder rasprodazha
global $PHP_SELF;
$dproducts=array();
...
if (isset($parent_array)){unset($parent_array);}
foreach(array_values($c2c[0]) as $cid){
    if (isset($c2c[$cid])){
	$to_process=array_values($c2c[$cid]);
	}else{unset($to_process);$parent_array[$cid][]=$cid;}
...
$query=tep_db_query('select sale_deduction_value_2,sale_deduction_value,sale_id,sale_categories_all,sale_quantity from salemaker_sales where sale_status=1 and sale_date_start<now()
...
$dQuery='select distinct p2c.categories_id,p.products_id,p.products_price as price,
	if (products_quantity>='.$data['qty'].','.$data['sdv2'].','.$data['sdv'].') as sdv
	from products p
	left join 
	...
	$dQuery.=' and p.products_quantity>0  having sdv>0 ';
	$query=tep_db_query($dQuery);
	if ($query!==false){
    	    while($res=tep_db_fetch_array($query)){
    	    $display_cat[$pp['child'][$res['categories_id']]]=1;
...
unset($display_cat['']);
//-----calculate categories to be displayed----
if ($PHP_SELF==FILENAME_DISCOUNT){//----only for discount page
$vPath=(int)$_REQUEST['vPath'];
$query_suffix=' and p.products_quantity>0 ';
if (isset($_REQUEST['srmi'])&&isset($_REQUEST['srma'])){
    $query_suffix.=' and p.products_price>='.(int)$_REQUEST['srmi'].' and p.products_price<='.(int)$_REQUEST['srma'];
    }else {$query_suffix.=' and p.products_price>0 ';}
if (isset($_REQUEST['filter_id'])&&(strlen($_REQUEST['filter_id'])>0)){
        $query_suffix.=' and  p.manufacturers_id='.(int)$_REQUEST['filter_id'].' ';
        }
if (isset($_REQUEST['vPath'])&&(strlen($_REQUEST['vPath'])>0)){
    if (isset($pp['parent'][$vPath])){
    $query_suffix.=' and p2c.categories_id in ('.implode(',',$pp['parent'][$vPath]).')';
	}
    }
/*-----having special prices------*/
$dQuery='select 
    s.products_id, p2c.categories_id,p.products_price as price,
    p.manufacturers_id
    from specials s
    ...
    where  s.status = 1 and (s.expires_date > NOW() or s.expires_date is null)';
$dQuery.=$query_suffix;
$min_price=1000000;$max_price=0;
$query=tep_db_query($dQuery);
if ($query!==false){
    while($res=tep_db_fetch_array($query)){
	$dproducts[$res['products_id']]=1;
	$dmanufacturers[$res['manufacturers_id']]=1;
	if ($res['price']>$max_price){$max_price=$res['price'];}
	if ($res['price']<$min_price){$min_price=$res['price'];}
	}
}
foreach($qty as $quantity=>$data){
$dQuery='select distinct p2c.categories_id,p.products_id,p.products_price as price,
	...
	left join 
        ...
        left join
	   manufacturers m ON (m.manufacturers_id = p.manufacturers_id)
	where  p.products_status=1 '.$query_suffix;
	if (sizeof($data['categories'])>0){
	    $dQuery.=' and p2c.categories_id in ('.implode(',',$data['categories']).') ';
	    }
	$dQuery.=' having sdv>0 ';
...
$dQuery='select distinct p2c.categories_id,p.products_id,p.products_price as price,
	...
	if (isset($_REQUEST['vPath'])&&(strlen($_REQUEST['vPath'])>0)){
	    if (isset($pp['parent'][$vPath])){
	    $dQuery.=' and p2c.categories_id in ('.implode(',',$pp['parent'][$vPath]).')';
		}
	    }
	if (sizeof($data['categories'])>0){
	    $dQuery.=' and p2c.categories_id in ('.implode(',',$data['categories']).') ';
...
/*----define manufacturers----*/        
}//----only for discount page
unset($dmanufacturers['']);
$ret=array('products'=>$dproducts,'categories'=>$display_cat,'min'=>$min_price,'max'=>$max_price,'manufacturers'=>$dmanufacturers);
return $ret;
}
?>