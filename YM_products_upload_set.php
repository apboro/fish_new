...
class YandexMarket
{
    ...
    on pc.products_id = p.products_id
        ...
        foreach ($products as $product) {
            ...
            //$suppliers_id_list = array(17, 15, 18, 26, 56, 64); //выбираем поставщиков
            $manufacturers_id_list = array(49,37,1078, 781, 84, 914, 250, 272, 83, 830, 956);
            if ((!in_array($product['products_id'], $ar)) and ($product['products_price'] > 2500)
                //and (in_array($product['supplier_id'], $suppliers_id_list))
                and (in_array($product['manufacturers_id'], $manufacturers_id_list))
            ) {
            ...
            $prods_to_feed = array(
                ...
                250946,                252277,
                253516,                256970,
                257064,                305357, 250941,
                // '4627078261407', 'FT900', "98623", "98626", "98627", "142978", "NAS1000FB",
                // "FC-1040FD", "CAIM02С", "CBVI42", "F185330", "   ", "FCDI130-420", "ARNSAXPR90", "KX00003301",
                // "8806066100188", "8806066100218", "8806066100225", "8806066101031", "025-0049", "025-0045", "W-A-442260",
            );
...
if (strripos($_SERVER['REQUEST_URI'], '.zip') == false) {
    $filename = 'yandex_market4.gz';
} else {
    $filename = 'yandex_market4.zip';
}
...
file_put_contents('yandex_market_tmp4.xml');
//shell_exec('cd ' . DIR_FS_CACHE . ' && rm -f yandex_market4.zip && zip yandex_market4.zip yandex_market_tmp4.xml');
//shell_exec('cd ' . DIR_FS_CACHE . ' && rm -f yandex_market4.gz && gzip -c yandex_market_tmp4.xml > yandex_market4.gz');
