...
if (isset($_POST['supd'])) {
    $_GET['action'] = 'update_category_order';
}
if (isset($_POST['qupd'])) {
    $_GET['action'] = 'update_quantity';
}
if (isset($_POST['gupd'])) {
    $_GET['action'] = 'update_group';
}
...
if (true) {
//    echo "<script>alert('".print_r($_GET)."');</script>";
...
//Added for Categories Description 1.5
        ...
        case 'edit_category':
            ...
            break;
//End Categories Description 1.5
//----by iHolder update products_quantity
        case 'update_quantity': {
            foreach ($_POST['qprid'] as $prid => $qty) {
                $sql_data_array = array('products_quantity' => $qty);
                tep_db_perform(TABLE_PRODUCTS, $sql_data_array, 'update', "products_id = " . (int)$prid);
            }
        }
            break;
        case 'update_group': {
            ...
            foreach ($_POST['group'] as $id => $group) {
                $group_ind = (int)$group;
                $sql_data_array = array('products_group' => $group_ind);
                tep_db_perform(TABLE_PRODUCTS, $sql_data_array, 'update', "products_id = " . (int)$id);
            }
            foreach ($_POST['model'] as $id => $model) {
                $sql_data_array = array('products_model_tag' => $model);
                tep_db_perform(TABLE_PRODUCTS, $sql_data_array, 'update', "products_id = " . (int)$id);
            }
        }
            break;
...
//==================================================================================================
        if (isset($_POST['product_id'])) {
            foreach ($_POST['product_id'] as $product_id => $pos) {
                $products_id = $product_id;
                $sort_order = $pos;
//        $sql_data_array = array('sort_order' => $sort_order);
// BOF Enable - Disable Categories Contribution--------------------------------------
                $sql_data_array = array('products_sort_order' => $sort_order);
// EOF Enable - Disable Categories Contribution--------------------------------------
            ...
            tep_redirect(tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $products_id));
            break;
        }
//==================================================================================================
...
// BOF Enable - Disable Categories Contribution--------------------------------------
                ...
                $hide_menu = tep_db_prepare_input($_POST['hide_menu']);
...
// EOF: KategorienAdmin / OLI
                ...
                for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
                    ...
                    $categories_yandex_name = $_POST['$categories_yandex_name'];
                    ...
                    $categories_og_title_array = $_POST['categories_og_title'];
                    $categories_og_description_array = $_POST['categories_og_description'];
                    $categories_tk_title_array = $_POST['categories_tk_title'];
                    $categories_tk_description_array = $_POST['categories_tk_description'];

                    $categories_og_image_array = $_POST['categories_og_image'];
                    $categories_tk_image_array = $_POST['categories_tk_image'];

                    $language_id = $languages[$i]['id'];

                    $sql_data_array = array('categories_name' => tep_db_prepare_input($categories_name_array[$language_id]),
                        'categories_yandex_name' => tep_db_prepare_input($categories_yandex_array[$language_id]),
                        ...
                        'og_title' => tep_db_prepare_input($categories_og_title_array[$language_id]),
                        'og_description' => tep_db_prepare_input($categories_og_description_array[$language_id]),
                        'tk_title' => tep_db_prepare_input($categories_tk_title_array[$language_id]),
                        'tk_description' => tep_db_prepare_input($categories_tk_description_array[$language_id]),
                        'og_image' => tep_db_prepare_input($categories_og_image_array[$language_id]),
                        'tk_image' => tep_db_prepare_input($categories_tk_image_array[$language_id])
...
//Added for Categories Description 1.5
                    if (ALLOW_CATEGORY_DESCRIPTIONS == 'true') {
                        $sql_data_array = array('categories_name' => tep_db_prepare_input($_POST['categories_name'][$language_id]),
                            'categories_yandex_name' => tep_db_prepare_input($_POST['categories_yandex_name'][$language_id]),
                            ...
                            'categories_bottom_name' => tep_db_prepare_input($_POST['categories_bottom_name'][$language_id]),
                            'categories_top_name' => tep_db_prepare_input($_POST['categories_top_name'][$language_id]),
                            'categories_top_content' => tep_db_prepare_input($_POST['categories_top_content'][$language_id]),
                            'categories_bottom_content' => tep_db_prepare_input($_POST['categories_bottom_content'][$language_id]),
                            'og_title' => tep_db_prepare_input($_POST['categories_og_title'][$language_id]),
                            'og_description' => tep_db_prepare_input($_POST['categories_og_description'][$language_id]),
                            'tk_title' => tep_db_prepare_input($_POST['categories_tk_title'][$language_id]),
                            'tk_description' => tep_db_prepare_input($_POST['categories_tk_description'][$language_id]),
                            'og_image' => tep_db_prepare_input($_POST['categories_og_image'][$language_id]),
                            'tk_image' => tep_db_prepare_input($_POST['categories_tk_image'][$language_id])
                            /*by iHolder---*/
                        );
                    }
//End Categories Description 1.5
...
//            }
            ...
            if (isset($_POST['categories_id']) || isset($_GET['cPath']) ) {
                echo "test2";
                $cats=array();

                if( isset($_POST['categories_id'])) $cats[]=$_POST['categories_id'];
                if( isset($_GET['cPath'])) $cats[]=$_GET['cPath'];

                foreach ($cats as $cat_id => $e_id) {
...
// EOF: KategorienAdmin / OLI
                    ...
                    for ($i = 0, $n = sizeof($categories); $i < $n; $i++) {
                        $product_ids_query = tep_db_query("select products_id from " . TABLE_PRODUCTS_TO_CATEGORIES . " where categories_id = '" . (int)$categories_id . "'");

                        while ($product_ids = tep_db_fetch_array($product_ids_query)) {
                            $products[$product_ids['products_id']]['categories'][] = $categories_id;
                        }
                    }
...
// removing categories can be a lengthy process
            ...
            if (isset($_POST['products_id'])) {
                ...
                $categories_id = tep_db_prepare_input($_POST['prod_id']);
...
// EOF MaxiDVD: Modified For Ultimate Images Pack!
                ...
                $sql_data_array = array('products_quantity' => (int)tep_db_prepare_input($_POST['products_quantity']),
                    'products_group' => (int)tep_db_prepare_input($_POST['products_group']),
                    ...
                    'supplier_id' => (int)tep_db_prepare_input($_POST['supplier_id']),
                    'products_sort_order' => tep_db_prepare_input($_POST['products_sort_order']),
                    ...
                    'products_purchase_price' => tep_db_prepare_input($_POST['products_purchase_price']));
...
// End Products Specifications
                        ...
                        'products_bottom_name' => tep_db_prepare_input($_POST['products_bottom_name'][$language_id]),
                        'products_top_name' => tep_db_prepare_input($_POST['products_top_name'][$language_id]),
                        'products_top_content' => tep_db_prepare_input($_POST['products_top_content'][$language_id]),
                        'products_bottom_content' => tep_db_prepare_input($_POST['products_bottom_content'][$language_id]),
                        'og_title' => tep_db_prepare_input($_POST['products_og_title'][$language_id]),
                        'og_description' => tep_db_prepare_input($_POST['products_og_description'][$language_id]),
                        'tk_title' => tep_db_prepare_input($_POST['products_tk_title'][$language_id]),
                        'tk_description' => tep_db_prepare_input($_POST['products_tk_description'][$language_id]),
                        'og_image' => tep_db_prepare_input($_POST['products_og_image'][$language_id]),
                        'tk_image' => tep_db_prepare_input($_POST['products_tk_image'][$language_id])
                        /*--by iHolder--*/
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
		...
		tk_title,tk_description,
		tk_image,og_image,
		products_viewed) values ('" . (int)$dup_products_id . "', '" . (int)$description['language_id'] . "', '" . tep_db_input($description['products_name']) . "', '" . tep_db_input($description['products_info']) . "', '" . tep_db_input($description['products_description']) . "', '" . tep_db_input($description['products_head_title_tag']) . "', '" . tep_db_input($description['products_head_desc_tag']) . "', '" . tep_db_input($description['products_head_keywords_tag']) . "', '" . tep_db_input($description['products_url']) . "', '" . tep_db_input($description['products_tab_1']) . "', '" . tep_db_input($description['products_tab_2']) . "', '" . tep_db_input($description['products_tab_3']) . "', '" . tep_db_input($description['products_tab_4']) . "', '" . tep_db_input($description['products_tab_5']) . "', '" . tep_db_input($description['products_tab_6']) . "','" . tep_db_input($description['products_bottom_name']) . "','" . tep_db_input($description['products_top_name']) . "','" . tep_db_input($description['products_top_content']) . "','" . tep_db_input($description['products_bottom_content']) .
                            "', '" . tep_db_input($description['products_og_title']) . "', '" . tep_db_input($description['products_og_description']) .
                            "', '" . tep_db_input($description['products_tk_title']) . "', '" . tep_db_input($description['products_tk_description']) .
                            "', '" . tep_db_input($description['products_tk_image']) . "', '" . tep_db_input($description['products_og_image']) .
                            "','0')");
                    }
...
// copy image only if modified
        ...
        case 'update_category_order':
            if (isset($_POST['product_order_set'])) {
                foreach ($_POST['prod_id'] as $product_id => $pos) {
                    $products_id = $product_id;
                    $sort_order = (int)$_POST['product_order_set'];
//        $sql_data_array = array('sort_order' => $sort_order);
// BOF Enable - Disable Categories Contribution--------------------------------------
                    $sql_data_array = array('products_sort_order' => $sort_order);
// EOF Enable - Disable Categories Contribution--------------------------------------
...
?>
    ...
    <head>
        ...
        <script language="javascript"><!--
            ...
            function popupWindow2(url) {
                window.open(url, 'popupWindow', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,copyhistory=no,width=900,height=600')
            }
            //--></script>
        <script language="javascript"><!--
    ...
    <script language="javascript"><!--
        ...
        function setGroupOne(){
            jQuery('[name^=group]').val(1);
        }
        //--></script>
        ...
        <tr>
            ...
            <td width="100%" valign="top">
                <table class="table-padding-2">
                    ...
                    if (($_GET['cID']) && (!$_POST)) {
                        $categories_query = tep_db_query("select c.categories_id,c.hide_menu, c.categories_status, cd.categories_yandex_name,cd.categories_name, cd.categories_heading_title, cd.categories_description,  cd.categories_meta_title, cd.categories_meta_description, cd.categories_meta_keywords, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified,
	    cd.og_title as categories_og_title,cd.og_description as categories_og_description,
	    cd.tk_title as categories_tk_title,cd.tk_description as categories_tk_description,
	    cd.og_image as categories_og_image,cd.tk_image as categories_tk_image,
	    cd.categories_url,cd.categories_bottom_name,cd.categories_top_name,cd.categories_top_content,cd.categories_bottom_content
	    from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = '" . $_GET['cID'] . "' and c.categories_id = cd.categories_id and cd.language_id = '" . $languages_id . "' order by c.sort_order, cd.categories_name");
                        ...
                        $categories_bottom_name = $_POST['categories_bottom_name'];

                        $categories_url = $_POST['categories_url'];
                        $categories_top_name = $_POST['categories_top_name'];
                        $categories_top_content = $_POST['categories_top_content'];
                        $categories_bottom_content = $_POST['categories_bottom_content'];
                        $categories_og_title = $_POST['categories_og_title'];
                        $categories_og_description = $_POST['categories_og_description'];
                        $categories_tk_title = $_POST['categories_tk_title'];
                        $categories_tk_description = $_POST['categories_tk_description'];
                        $categories_og_image = $_POST['categories_og_image'];
                        $categories_tk_image = $_POST['categories_tk_image'];
                            ...
                            <div id="tabs">
                                ...
                                <ul>
                                    ...
                                    for ($i = 0; $i < sizeof($languages); $i++) {
                                        ?>
                                        <li><a href="#tab<?php echo $i; ?>"><?php echo $languages[$i]['name']; ?></a>
                                        </li>
                                        <?php
                                    }
                                    ?>
                                    ...
                                    <div id="tab<?php echo $i; ?>">
                                        <table border="0" class="main">
                                            ...
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_input_field('categories_name[' . $languages[$i]['id'] . ']', (($categories_name[$languages[$i]['id']]) ? stripslashes($categories_name[$languages[$i]['id']]) : tep_get_category_name($cInfo->categories_id, $languages[$i]['id'])), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main">Название в яндексе:</td>
                                                <td class="main"><?php echo tep_draw_input_field('categories_yandex_name[' . $languages[$i]['id'] . ']', (($categories_yandex_name[$languages[$i]['id']]) ? stripslashes($categories_yandex_name[$languages[$i]['id']]) : tep_get_category_yandex_name($cInfo->categories_id, $languages[$i]['id'])), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_input_field('categories_heading_title[' . $languages[$i]['id'] . ']', (($categories_name[$languages[$i]['id']]) ? stripslashes($categories_name[$languages[$i]['id']]) : tep_get_category_heading_title($cInfo->categories_id, $languages[$i]['id'])), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main"
                                                    valign="top"><?php echo TEXT_EDIT_CATEGORIES_DESCRIPTION; ?></td>
                                                <td>
                                            ...
                                            <!--iHolder changes---->
                                            <tr>
                                                <td class="main" valign="top"><b>Ссылка на категорию</b></td>
                                                <td class="main" valign="top">
                                                    <?php
                                                    echo tep_draw_input_field('categories_url[' . $languages[$i]['id'] . ']', (($categories_url[$languages[$i]['id']]) ? stripslashes($categories_url[$languages[$i]['id']]) : $cInfo->categories_url), 'size=110');
                                                    ?>
                                                </td>
                                            </tr>

                                            <tr>
                                                <td class="main" valign="top"><b>Название категории(вверху)</b></td>
                                                <td class="main" valign="top">
                                                    <?php
                                                    echo tep_draw_input_field('categories_top_name[' . $languages[$i]['id'] . ']', (($categories_top_name[$languages[$i]['id']]) ? stripslashes($categories_top_name[$languages[$i]['id']]) : $cInfo->categories_top_name), 'size=110');
                                                    ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="main" valign="top"><b>Содержимое категории(вверху)</b></td>
                                                <td class="main" valign="top">
                                                    <?php
                                                    echo tep_draw_textarea_field('categories_top_content[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($categories_top_content[$languages[$i]['id']]) ? stripslashes($categories_top_content[$languages[$i]['id']]) : $cInfo->categories_top_content));
                                                    ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="main" valign="top"><b>Название категории(внизу)</b></td>
                                                <td class="main" valign="top">
                                                    <?php
                                                    echo tep_draw_input_field('categories_bottom_name[' . $languages[$i]['id'] . ']', (($categories_bottom_name[$languages[$i]['id']]) ? stripslashes($categories_bottom_name[$languages[$i]['id']]) : $cInfo->categories_bottom_name), 'size=110');
                                                    ?>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="main" valign="top"><b>Содержимое категории(внизу)</b></td>
                                                <td class="main" valign="top">
                                                    <?php
                                                    echo tep_draw_textarea_field('categories_bottom_content[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($categories_bottom_content[$languages[$i]['id']]) ? stripslashes($categories_bottom_content[$languages[$i]['id']]) : $cInfo->categories_bottom_content));
                                                    ?>
                                                </td>
                                            </tr>
                                            ...
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_input_field('categories_meta_title[' . $languages[$i]['id'] . ']', (($categories_meta_title[$languages[$i]['id']]) ? stripslashes($categories_meta_title[$languages[$i]['id']]) : tep_get_category_meta_title($cInfo->categories_id, $languages[$i]['id'])), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_textarea_field('categories_meta_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($categories_meta_description[$languages[$i]['id']]) ? stripslashes($categories_meta_description[$languages[$i]['id']]) : tep_get_category_meta_description($cInfo->categories_id, $languages[$i]['id']))); ?></td>
                                            </tr>
                                            <tr>
                                                <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_input_field('categories_meta_keywords[' . $languages[$i]['id'] . ']', (($categories_meta_keywords[$languages[$i]['id']]) ? stripslashes($categories_meta_keywords[$languages[$i]['id']]) : tep_get_category_meta_keywords($cInfo->categories_id, $languages[$i]['id'])), 'size=120'); ?></td>
                                            </tr>

                                            <!-- /NEW META -->
                                        </table>
                                        <table style="width:100%;border:1px dashed green;background-color:#eee;">
                                            <tr>
                                                <td class="main">Open Graph Заголовок</td>
                                                <td class="main"><?php echo tep_draw_input_field('categories_og_title[' . $languages[$i]['id'] . ']', (($categories_og_title[$languages[$i]['id']]) ? stripslashes($categories_og_title[$languages[$i]['id']]) : $cInfo->categories_og_title), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main">Open Graph Описание</td>
                                                <td class="main"><?php echo tep_draw_textarea_field('categories_og_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($categories_og_description[$languages[$i]['id']]) ? stripslashes($categories_og_description[$languages[$i]['id']]) : $cInfo->categories_og_description)); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main">Open Graph картинка</td>
                                                <td class="main"><?php echo tep_draw_input_field('categories_og_image[' . $languages[$i]['id'] . ']', (($categories_og_image[$languages[$i]['id']]) ? stripslashes($categories_og_image[$languages[$i]['id']]) : $cInfo->categories_og_image), 'size=120'); ?></td>
                                            </tr>


                                            <tr>
                                                <td class="main">Twitter card Заголовок</td>
                                                <td class="main"><?php echo tep_draw_input_field('categories_tk_title[' . $languages[$i]['id'] . ']', (($categories_tk_title[$languages[$i]['id']]) ? stripslashes($categories_tk_title[$languages[$i]['id']]) : $cInfo->categories_tk_title), 'size=120'); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main">Twitter card Описание</td>
                                                <td class="main"><?php echo tep_draw_textarea_field('categories_tk_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (($categories_tk_description[$languages[$i]['id']]) ? stripslashes($categories_tk_description[$languages[$i]['id']]) : $cInfo->categories_tk_description)); ?></td>
                                            </tr>
                                            <tr>
                                                <td class="main">Twitter card картинка</td>
                                                <td class="main"><?php echo tep_draw_input_field('categories_tk_image[' . $languages[$i]['id'] . ']', (($categories_tk_image[$languages[$i]['id']]) ? stripslashes($categories_tk_image[$languages[$i]['id']]) : $cInfo->categories_tk_image), 'size=120'); ?></td>
                                            </tr>
                                ...
                                <div id="data">
                                    <table border="0" class="main">
                                        ...
                                        <tr>
                                            ...
                                            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('categories_status', $cInfo->categories_status, 'size="2"'); ?>
                                                &nbsp;<?php echo TEXT_DEFINE_CATEGORY_STATUS; ?></td>
                                        </tr>
                                        <tr>
                                            <td class="main"><?php echo TEXT_EDIT_HIDE_MENU; ?></td>
                                            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('hide_menu', $cInfo->hide_menu, 'size="2"'); ?>
                                        </tr>
...
#     }
                                ...
                                $category_query = tep_db_query("select cd.og_title,cd.og_description,
	cd.tk_title,cd.tk_description,
        cd.og_image,cd.tk_image,
	c.categories_id, cd.language_id, cd.categories_name, cd.categories_heading_title, cd.categories_description, cd.categories_meta_title, cd.categories_meta_description, cd.categories_meta_keywords, c.categories_image, c.sort_order, c.date_added, c.last_modified from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.categories_id = cd.categories_id and c.categories_id = '" . $_GET['cID'] . "'");
                    ...
                    <tr>
                        <td align="right" class="smallText">
                            ...
                            for ($i = 0; $i < sizeof($languages); $i++) {
                                ...
                                echo tep_draw_hidden_field('categories_yandex_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_yandex_name[$languages[$i]['id']])));
                                ...
                                echo tep_draw_hidden_field('categories_bottom_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_bottom_name[$languages[$i]['id']])));

                                echo tep_draw_hidden_field('categories_url[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_url[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_top_name[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_top_name[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_top_content[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_top_content[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_bottom_content[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_bottom_content[$languages[$i]['id']])));

                                echo tep_draw_hidden_field('categories_og_title[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_og_title[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_og_description[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_og_description[$languages[$i]['id']])));

                                echo tep_draw_hidden_field('categories_tk_title[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_tk_title[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_tk_description[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_tk_description[$languages[$i]['id']])));

                                echo tep_draw_hidden_field('categories_og_image[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_og_image[$languages[$i]['id']])));
                                echo tep_draw_hidden_field('categories_tk_image[' . $languages[$i]['id'] . ']', htmlspecialchars(stripslashes($categories_tk_image[$languages[$i]['id']])));
...
// BOF MaxiDVD: Modified For Ultimate Images Pack!
                    ...
                    $product_query = tep_db_query("select pd.products_name, p.supplier_id, pd.products_description, pd.products_head_title_tag, pd.products_head_desc_tag, pd.products_head_keywords_tag, pd.products_url, 
	pd.products_bottom_name,p.products_purchase_price,pd.products_top_name,
	pd.og_title as products_og_title,pd.og_description as products_og_description,
	pd.tk_title as products_tk_title,pd.tk_description as products_tk_description,
	pd.og_image as products_og_image,pd.tk_image as products_tk_image,
	pd.products_top_content,pd.products_bottom_content,
	p.products_id, p.products_quantity,p.products_group, p.products_model, p.products_image, p.products_image_med, p.products_image_lrg, p.products_image_sm_1, p.products_image_xl_1, p.products_image_sm_2, p.products_image_xl_2, p.products_image_sm_3, p.products_image_xl_3, p.products_image_sm_4, p.products_image_xl_4, p.products_image_sm_5, p.products_image_xl_5, p.products_image_sm_6, p.products_image_xl_6, " . $products_price_list . ", p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_to_xml, p.products_tax_class_id, p.products_quantity_order_min, p.products_quantity_order_units, p.products_sort_order, p.manufacturers_id,p.products_length,p.products_width,p.products_height from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$_GET['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
// EOF MaxiDVD: Modified For Ultimate Images Pack!
                    $product = tep_db_fetch_array($product_query);
...
// End Products Specifications
                    ...
                    $products_bottom_name = $_POST['products_bottom_name'];
                    $products_top_name = $_POST['products_top_name'];
                    $products_top_content = $_POST['products_top_content'];
                    $products_bottom_content = $_POST['products_bottom_content'];
                    $products_purchase_price = $_POST['products_purchase_price'];
                    $products_og_title = $_POST['products_og_title'];
                    $products_og_description = $_POST['products_og_description'];
                    $products_tk_title = $_POST['products_tk_title'];
                    $products_tk_description = $_POST['products_tk_description'];
                    $products_og_image = $_POST['products_og_image'];
                    $products_tk_image = $_POST['products_tk_image'];
                ...
                // BOF manufacturers descriptions
                //  $manufacturers_query = tep_db_query("select manufacturers_id, manufacturers_name from " . TABLE_MANUFACTURERS . " order by manufacturers_name");
                $suppliers_query = tep_db_query("select supplier_id, supplier_name from " . TABLE_SUPPLIER . " where 1 order by supplier_name");
                // EOF manufacturers descriptions
                while ($supplier = tep_db_fetch_array($suppliers_query)) {
                    $suppliers_array[] = array('id' => $supplier['supplier_id'],
                                ...
                                <div id="tabs">
                                    ...
                                    <ul>
                                        ...
                                        for ($i = 0; $i < sizeof($languages); $i++) {
                                            ?>
                                            <li>
                                                <a href="#tab<?php echo $i; ?>"><?php echo $languages[$i]['name']; ?></a>
                                            </li>
                                            <?php
                                        }
                                        ?>
                                    ...
                                    <div id="data">
                                        <table border="0">
                                            ...
                                            <tr>
                                                ...
                                                <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('supplier_id', $suppliers_array, $pInfo->supplier_id); ?></td>
                                            </tr>
                                            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
                                            </tr>
                                            <tr>
                                            ...
                                            while ($extra_fields = tep_db_fetch_array($extra_fields_query)) {
                                                ...
                                                ?>
                                                <tr bgcolor="#ebebff">
                                                    <td class="main"><?php echo $extra_fields['products_extra_fields_name']; ?>
                                                        :
                                                    </td>
                                    ...
                                    <?php for ($i = 0, $n = sizeof($languages); $i < $n; $i++) { ?>
                                        ...
                                        <div id="tab<?php echo $i; ?>">
                                            <table border="0" class="main">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <table border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                                <td class="main"
                                                                    valign="top"><?php echo tep_draw_textarea_field('products_info[' . $languages[$i]['id'] . ']', 'soft', '70', '10', (isset($products_info[$languages[$i]['id']]) ? $products_info[$languages[$i]['id']] : tep_get_products_info($pInfo->products_id, $languages[$i]['id'])), 'class="notinymce"'); ?></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="main"
                                                        valign="top"><?php echo TEXT_PRODUCTS_DESCRIPTION; ?></td>
                                                    <td>
                                                        <table border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                                <td class="main">
                                                ...
                                                <!--by iHolder-->
                                                <tr>
                                                    <td class="main" valign="top"><b>Название товара(сверху)</b></td>
                                                    <td><?php echo tep_draw_input_field('products_top_name[' . $languages[$i]['id'] . ']', (isset($products_top_name[$languages[$i]['id']]) ? stripslashes($products_top_name[$languages[$i]['id']]) : $pInfo->products_top_name), 'size=110'); ?>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="main" valign="top"><b>Контент товара(сверху)</b></td>
                                                    <td>
                                                        <?php
                                                        echo tep_draw_textarea_field('products_top_content[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (isset($products_top_content[$languages[$i]['id']]) ? stripslashes($products_top_content[$languages[$i]['id']]) : $pInfo->products_top_content));
                                                        ?>

                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="main" valign="top"><b>Название товара(снизу)</b></td>
                                                    <td><?php echo tep_draw_input_field('products_bottom_name[' . $languages[$i]['id'] . ']', (isset($products_bottom_name[$languages[$i]['id']]) ? stripslashes($products_bottom_name[$languages[$i]['id']]) : $pInfo->products_bottom_name), 'size=110'); ?>
                                                    </td>
                                                </tr>

                                                <tr>
                                                    <td class="main" valign="top"><b>Контент товара(внизу)</b></td>
                                                    <td>
                                                        <?php
                                                        echo tep_draw_textarea_field('products_bottom_content[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (isset($products_bottom_content[$languages[$i]['id']]) ? stripslashes($products_bottom_content[$languages[$i]['id']]) : $pInfo->products_bottom_content));
                                                        ?>
                                                ...
                                                <tr>
                                                    <td colspan="2">
                                                        <table style="width:100%;background-color:#eee;border:1px solid green">
                                                            <tr>
                                                                <td class="main" valign="top"><b>Open Graph
                                                                        заголовок</b></td>
                                                                <td><?php echo tep_draw_input_field('products_og_title[' . $languages[$i]['id'] . ']', (isset($products_og_title[$languages[$i]['id']]) ? stripslashes($products_og_title[$languages[$i]['id']]) : $pInfo->products_og_title), 'size=110'); ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="main" valign="top"><b>Open Graph описание</b>
                                                                </td>
                                                                <td><?php echo tep_draw_textarea_field('products_og_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (isset($products_og_description[$languages[$i]['id']]) ? stripslashes($products_og_description[$languages[$i]['id']]) : $pInfo->products_og_description)); ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="main" valign="top"><b>Open Graph картинка</b>
                                                                </td>
                                                                <td><?php echo tep_draw_input_field('products_og_image[' . $languages[$i]['id'] . ']', (isset($products_og_image[$languages[$i]['id']]) ? stripslashes($products_og_image[$languages[$i]['id']]) : $pInfo->products_og_image), 'size=110'); ?></td>
                                                            </tr>


                                                            <tr>
                                                                <td class="main" valign="top"><b>Twitter card
                                                                        заголовок</b></td>
                                                                <td><?php echo tep_draw_input_field('products_tk_title[' . $languages[$i]['id'] . ']', (isset($products_tk_title[$languages[$i]['id']]) ? stripslashes($products_tk_title[$languages[$i]['id']]) : $pInfo->products_tk_title), 'size=110'); ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="main" valign="top"><b>Twitter card
                                                                        описание</b></td>
                                                                <td><?php echo tep_draw_textarea_field('products_tk_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (isset($products_tk_description[$languages[$i]['id']]) ? stripslashes($products_tk_description[$languages[$i]['id']]) : $pInfo->products_tk_description)); ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="main" valign="top"><b>Twitter card
                                                                        картинка</b></td>
                                                                <td><?php echo tep_draw_input_field('products_tk_image[' . $languages[$i]['id'] . ']', (isset($products_tk_image[$languages[$i]['id']]) ? stripslashes($products_tk_image[$languages[$i]['id']]) : $pInfo->products_tk_image), 'size=110'); ?></td>
                                                            </tr>
                                                ...
                                                <tr>
                                                    <td class="main"
                                                        valign="top"><?php echo TEXT_PRODUCTS_PAGE_TITLE; ?></td>
                                                    <td>
                                                        <table border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                                <td class="main"
                                                                    valign="top"><?php echo tep_draw_textarea_field('products_head_title_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '5', (isset($products_head_title_tag[$languages[$i]['id']]) ? $products_head_title_tag[$languages[$i]['id']] : tep_get_products_head_title_tag($pInfo->products_id, $languages[$i]['id'])), 'class="notinymce"'); ?></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
                                                ...
                                                <tr>
                                                    <td class="main"
                                                        valign="top"><?php echo TEXT_PRODUCTS_HEADER_DESCRIPTION; ?></td>
                                                    <td>
                                                        <table border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                                <td class="main"
                                                                    valign="top"><?php echo tep_draw_textarea_field('products_head_desc_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '5', (isset($products_head_desc_tag[$languages[$i]['id']]) ? $products_head_desc_tag[$languages[$i]['id']] : tep_get_products_head_desc_tag($pInfo->products_id, $languages[$i]['id'])), 'class="notinymce"'); ?></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
                                                ...
                                                <tr>
                                                    <td class="main"
                                                        valign="top"><?php echo TEXT_PRODUCTS_KEYWORDS; ?></td>
                                                    <td>
                                                        <table border="0" cellspacing="0" cellpadding="0">
                                                            <tr>
                                                                <td class="main"
                                                                    valign="top"><?php echo tep_draw_textarea_field('products_head_keywords_tag[' . $languages[$i]['id'] . ']', 'soft', '70', '5', (isset($products_head_keywords_tag[$languages[$i]['id']]) ? $products_head_keywords_tag[$languages[$i]['id']] : tep_get_products_head_keywords_tag($pInfo->products_id, $languages[$i]['id'])), 'class="notinymce"'); ?></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                                </tr>
                                    ...
                                    <div id="images">
                                        <table border="0">
                                            ...
                                            ?>
                                            <tr>
                                                <td class="dataTableRow" width="30%" valign="top"><span
                                                            class="main"><?php echo TEXT_PRODUCTS_IMAGE_NOTE; ?></span>
                                                </td>
                                                <td class="dataTableRow" width="70%" valign="top"><span
                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image') . '&nbsp;'; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir', $image_directories, dirname($pInfo->products_image) . '/'); ?>
                                                        <br>
                                                        <?php if (($_GET['pID']) && ($pInfo->products_image) != '')
                                                            echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image, $pInfo->products_image, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . tep_draw_hidden_field('products_previous_image', $pInfo->products_image) . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="dataTableRow" width="30%" valign="top"><span
                                                            class="main"><?php echo TEXT_PRODUCTS_IMAGE_MEDIUM; ?></span>
                                                </td>
                                                <td class="dataTableRow" width="70%" valign="top"><span
                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_med') . '&nbsp;'; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_med', $image_directories, dirname($pInfo->products_image_med) . '/'); ?>
                                                        <br>
                                                        <?php if (($_GET['pID']) && ($pInfo->products_image_med) != '')
                                                            echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image_med . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_med, $pInfo->products_image_med, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . tep_draw_hidden_field('products_previous_image_med', $pInfo->products_image_med) . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_med" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_med" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="dataTableRow" width="30%" valign="top"><span
                                                            class="main"><?php echo TEXT_PRODUCTS_IMAGE_LARGE; ?></span>
                                                </td>
                                                <td class="dataTableRow" width="70%" valign="top"><span
                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_lrg') . '&nbsp;'; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_lrg', $image_directories, dirname($pInfo->products_image_lrg) . '/'); ?>
                                                        <br>
                                                        <?php if (($_GET['pID']) && ($pInfo->products_image_lrg) != '')
                                                            echo tep_draw_separator('pixel_trans.gif', '24', '17" align="left') . $pInfo->products_image_lrg . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_lrg, $pInfo->products_image_lrg, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . tep_draw_hidden_field('products_previous_image_lrg', $pInfo->products_image_lrg) . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_lrg" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_lrg" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span>
                                                </td>
                                            </tr>
                                            ...
                                            if (ULTIMATE_ADDITIONAL_IMAGES == 'enable') {
                                                ...
                                                <tr>
                                                    <td class="main"
                                                        colspan="3"><?php echo TEXT_PRODUCTS_IMAGE_ADDITIONAL . '<br><hr>'; ?></td>
                                                </tr>
                                                <tr>
                                                        ...
                                                        <table border="0" cellpadding="2" cellspacing="0" width="100%">
                                                            <tr>
                                                                <td class="smalltext" colspan="2" valign="top"
                                                                    width="50%"><?php echo TEXT_PRODUCTS_IMAGE_TH_NOTICE; ?></td>
                                                                <td class="smalltext" colspan="2" valign="top"
                                                                    width="50%"><?php echo TEXT_PRODUCTS_IMAGE_XL_NOTICE; ?></td>
                                                            </tr>

                                                            <tr>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_1; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_1') . tep_draw_hidden_field('products_previous_image_sm_1', $pInfo->products_image_sm_1); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_1', $image_directories, dirname($pInfo->products_image_sm_1) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_1; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_1') . tep_draw_hidden_field('products_previous_image_xl_1', $pInfo->products_image_xl_1); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_1', $image_directories, dirname($pInfo->products_image_xl_1) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"><?php if (($_GET['pID']) && ($pInfo->products_image_sm_1) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_1)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_sm_1 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_1, $pInfo->products_image_sm_1, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_1" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_1" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_1) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_1)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_xl_1 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_1, $pInfo->products_image_xl_1, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_1" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_1" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_2; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_2') . tep_draw_hidden_field('products_previous_image_sm_2', $pInfo->products_image_sm_2); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_2', $image_directories, dirname($pInfo->products_image_sm_2) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_2; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_2') . tep_draw_hidden_field('products_previous_image_xl_2', $pInfo->products_image_xl_2); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_2', $image_directories, dirname($pInfo->products_image_xl_2) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_sm_2) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_2)) { ?><?php echo $pInfo->products_image_sm_2 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_2, $pInfo->products_name_sm_2, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_2" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_2" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_2) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_2)) { ?><?php echo $pInfo->products_image_xl_2 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_2, $pInfo->products_name_xl_2, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_2" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_2" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_3; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_3') . tep_draw_hidden_field('products_previous_image_sm_3', $pInfo->products_image_sm_3); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_3', $image_directories, dirname($pInfo->products_image_sm_3) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_3; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_3') . tep_draw_hidden_field('products_previous_image_xl_3', $pInfo->products_image_xl_3); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_3', $image_directories, dirname($pInfo->products_image_xl_3) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"><?php if (($_GET['pID']) && ($pInfo->products_image_sm_3) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_3)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_sm_3 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_3, $pInfo->products_name_sm_3, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_3" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_3" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_3) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_3)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_xl_3 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_3, $pInfo->products_name_xl_3, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_3" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_3" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_4; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_4') . tep_draw_hidden_field('products_previous_image_sm_4', $pInfo->products_image_sm_4); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_4', $image_directories, dirname($pInfo->products_image_sm_4) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_4; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_4') . tep_draw_hidden_field('products_previous_image_xl_4', $pInfo->products_image_xl_4); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_4', $image_directories, dirname($pInfo->products_image_xl_4) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_sm_4) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_4)) { ?><?php echo $pInfo->products_image_sm_4 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_4, $pInfo->products_name_sm_4, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_4" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_4" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_4) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_4)) { ?><?php echo $pInfo->products_image_xl_4 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_4, $pInfo->products_name_xl_4, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_4" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_4" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_5; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_5') . tep_draw_hidden_field('products_previous_image_sm_5', $pInfo->products_image_sm_5); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_5', $image_directories, dirname($pInfo->products_image_sm_5) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_5; ?></span>
                                                                </td>
                                                                <td class="dataTableRow" valign="top"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_5') . tep_draw_hidden_field('products_previous_image_xl_5', $pInfo->products_image_xl_5); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_5', $image_directories, dirname($pInfo->products_image_xl_5) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"> <?php if (($_GET['pID']) && ($pInfo->products_image_sm_5) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_5)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_sm_5 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_5, $pInfo->products_name_sm_5, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_5" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_5" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                                <td class="dataTableRow" colspan="2"
                                                                    valign="top"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_5) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_5)) { ?>
                                                                        <span class="smallText"><?php echo $pInfo->products_image_xl_5 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_5, $pInfo->products_name_xl_5, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_5" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_5" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?></span><?php }
                                                                    } ?></td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_SM_6; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_sm_6') . tep_draw_hidden_field('products_previous_image_sm_6', $pInfo->products_image_sm_6); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_sm_6', $image_directories, dirname($pInfo->products_image_sm_6) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo TEXT_PRODUCTS_IMAGE_XL_6; ?></span>
                                                                </td>
                                                                <td class="smallText top-valign"><span
                                                                            class="smallText"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_file_field('products_image_xl_6') . tep_draw_hidden_field('products_previous_image_xl_6', $pInfo->products_image_xl_6); ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . TEXT_PRODUCTS_IMAGE_DIR; ?>
                                                                        <br><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_pull_down_menu('img_dir_xl_6', $image_directories, dirname($pInfo->products_image_xl_6) . '/'); ?>
                                                                        <br>
                                                                </td>
                                                            </tr>
                                                            <tr>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_sm_6) != '') { ?><?php if (tep_not_null($pInfo->products_image_sm_6)) { ?><?php echo $pInfo->products_image_sm_6 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_sm_6, $pInfo->products_name_sm_6, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_sm_6" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_sm_6" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                                <td class="smallText top-valign"
                                                                    colspan="2"><?php if (($_GET['pID']) && ($pInfo->products_image_xl_6) != '') { ?><?php if (tep_not_null($pInfo->products_image_xl_6)) { ?><?php echo $pInfo->products_image_xl_6 . '<br>' . tep_image(DIR_WS_CATALOG_IMAGES . $pInfo->products_image_xl_6, $pInfo->products_name_xl_6, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'align="left" hspace="0" vspace="5"') . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="unlink_image_xl_6" value="yes">' . TEXT_PRODUCTS_IMAGE_REMOVE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '5', '15') . '&nbsp;<input type="checkbox" name="delete_image_xl_6" value="yes">' . TEXT_PRODUCTS_IMAGE_DELETE_SHORT . '<br>' . tep_draw_separator('pixel_trans.gif', '1', '42'); ?><?php }
                                                                    } ?></td>
                                                            </tr>
                                                        </table>
                                                    </td>
                                        ...
                                        <table border="0">
                                            ...
                                            <tr>
                                                <td class="main" colspan="2">
                                                    <br><?php echo ENTRY_PRODUCTS_PRICE . " 1"; ?></td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                <td class="main">Закупочная цена</td>
                                                <td class="main">
                                                    <?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' .
                                                        tep_draw_input_field('products_purchase_price', $pInfo->products_purchase_price); ?>
                                                </td>
                                            </tr>
                                            <?php
                                            $prices_num = tep_xppp_getpricesnum();
                                            for ($i = 2; $i <= $prices_num; $i++) { ?>
                                                ...
                                                <tr>
                                                    <td class="main" colspan="2">
                                                        <br><?php echo ENTRY_PRODUCTS_PRICE . " " . $i; ?>&nbsp;<input
                                                                type="checkbox"
                                                            ...
                                                            <td>
                                                                <table border="0" cellpadding="0" cellspacing="0"
                                                                    ...
                                                                    <tr class="dataTableHeadingRow">
                                                                        <td colspan="9" class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_1 ?></td>
                                                                    </tr>
                                                                    <tr class="dataTableHeadingRow">
                                                                        <td class="attributeBoxContent" width="300"
                                                                            align="left"><?php echo $options['products_options_name']; ?></td>
                                                                        <td class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_2 ?></td>
                                                                        <td class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_3 ?></td>
                                                                        <td class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_2 ?></td>
                                                                        <td class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_9 ?></td>
                                                                        <td class="attributeBoxContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_4 ?></td>
                                                                        <?php if (DOWNLOAD_ENABLED == 'true') { ?>
                                                                            <td class="attributeBoxContent"
                                                                                align="center"><?php echo TABLE_HEADING_ATTRIBUTE_5 ?></td>
                                                                            <td class="attributeBoxContent"
                                                                                align="center"><?php echo TABLE_HEADING_ATTRIBUTE_6 ?></td>
                                                                            <td class="attributeBoxContent"
                                                                                align="center"><?php echo TABLE_HEADING_ATTRIBUTE_7 ?></td>
                                                                            <td class="attributeBoxContent"
                                                                                align="center">&nbsp;</td>
                                                                        <?php } ?>
                                                                    </tr>
                                                                    ...
                                                                    <tr class="dataTableRow">
                                                                        <td class="dataTableContent"
                                                                            width="300"><?php echo tep_draw_checkbox_field('option[' . $rows . ']', $attributes['products_attributes_id'], $attributes['products_attributes_id']) . '&nbsp;' . $values['products_options_values_name']; ?>
                                                                            &nbsp;</td>
                                                                        <td class="dataTableContent" align="center">
                                                                            <select name="prefix[<?php echo $rows ?>]">
                                                                                <option value="+" <?php echo $posCheck ?>>
                                                                                    +
                                                                                <option value="-" <?php echo $negCheck ?>>
                                                                                    -
                                                                            </select></td>
                                                                        <td class="dataTableContent"
                                                                            align="center"><?php echo tep_draw_input_field('price[' . $rows . ']', $attributes['options_values_price'], 'size="7"'); ?></td>
                                                                        <td class="dataTableContent" align="center">
                                                                            <select name="products_attributes_weight_prefix[<?php echo $rows ?>]">
                                                                                <option value="+" <?php echo $wposCheck ?>>
                                                                                    +
                                                                                <option value="-" <?php echo $wnegCheck ?>>
                                                                                    -
                                                                            </select></td>
                                                                        <td class="dataTableContent"
                                                                            align="center"><?php echo tep_draw_input_field('products_attributes_weight[' . $rows . ']', $attributes['products_attributes_weight'], 'size="4"'); ?></td>
                                                                        <td class="dataTableContent"
                                                                            align="center"><?php echo tep_draw_input_field('products_options_sort_order[' . $rows . ']', $attributes['products_options_sort_order'], 'size="2"'); ?></td>
                                                                        <?php if (DOWNLOAD_ENABLED == 'true') { ?>
                                                                            <td class="dataTableContent"
                                                                                align="center"><?php echo tep_draw_input_field('filename[' . $rows . ']', $attributes_download['products_attributes_filename'], 'size="12" id=filename[' . $rows . ']'); ?></td>
                                                                            <td class="dataTableContent"
                                                                                align="center"><?php echo tep_draw_input_field('maxdays[' . $rows . ']', $attributes_download['products_attributes_maxdays'], 'size="2"'); ?></td>
                                                                            <td class="dataTableContent"
                                                                                align="center"><?php echo tep_draw_input_field('maxcount[' . $rows . ']', $attributes_download['products_attributes_maxcount'], 'size="2"'); ?></td>
                                                                            <td class="dataTableContent"
                                                                                align="center"><?php echo TABLE_TEXT_IS_PIN; ?> <?php echo tep_draw_checkbox_field('ispin[' . $rows . ']', '', $attributes_download['products_attributes_is_pin'], 1); ?>
                                                                                &nbsp;</td>
                                                                        <?php } ?>
                                                                    </tr>
                                                                    <?php
...
// End Products Specifications
                                            $pInfo->supplier_id = tep_db_prepare_input($pInfo->supplier_id, $languages[$i]['id']);
                                        ...
                                        <tr>
                                            <td class="main">
                                                <?php if ($products_image_med_name) { ?><?php echo tep_image(DIR_WS_CATALOG_IMAGES . $products_image_med_name, $products_image_med_name, MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT, 'align="right" hspace="5" vspace="5"');
                                        ...
                                        <tr>
                                            <td class="main"><b><u><?php echo sprintf(TEXT_PRODUCTS_INFO); ?></u></b>
                                            </td>
                                                                ...
                                                                <tr>
                                                                    <td class="smallText" align="right">
                                                                       ...
                                                                       /* if (is_array($admin_cat_access_array_cats) && (in_array("ALL", $admin_cat_access_array_cats) == false) && (pos($admin_cat_access_array_cats) != "")) {
                                                                            ...
                                                                            echo tep_draw_form('goto', FILENAME_CATEGORIES, '', 'get');
                                                                            $result = tep_db_query("select categories_id from `categories` Where hide_menu=1");
                                                                            $exclude = 0;
                                                                            while ($element = tep_db_fetch_array($result)) {
                                                                                $exclude = $element['categories_id'];
                                                                            }
                                                                             echo HEADING_TITLE_GOTO . ' ' . tep_draw_pull_down_menu('cPath', tep_get_category_tree('0','',$exclude), $current_category_id, 'onChange="this.form.submit();"');
                                            ...
                                            <td>
                                                <table class="table-padding-0">
                                                    <tr>
                                                        ...
                                                        <td valign="top">
                                                            <form id="categories-table" name="emailForm"
                                                                  action="?action=delete_category_confirm&cPath=<?php echo $_GET['cPath'] ?>"
                                                                  method="post" onSubmit="return yesqw();">
                                                                ...
                                                                <table class="table-padding-2">
                                                                    <tr class="dataTableHeadingRow">
                                                                        ...
                                                                        <td class="dataTableHeadingContent"><input
                                                                                    type="checkbox"
                                                                                    onClick="javascript:CheckAll(this.checked);">&nbsp;<?php echo TABLE_HEADING_CATEGORIES_PRODUCTS; ?>
                                                                        </td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_GROUP; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_MODEL; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_STATUS; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_XML; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_PRODUCT_SORT; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center"><?php echo TABLE_HEADING_ATTRIBUTE_10; ?></td>
                                                                        <td class="dataTableHeadingContent"
                                                                            align="center">&nbsp;</td>
                                                                    </tr>
                                                                    <?php
...
// EOF: KategorienAdmin / OLISWISS
                                                                            ...
                                                                            <td></td>
                                                                            <td class="dataTableContent">
                                                                                <input name="categories_id2[<?php echo $qaz; ?>]"
                                                                                       type="checkbox"
                                                                                       value="<?php echo $categories['categories_id'];
                                                                                       $qaz++; ?>">&nbsp;

                                                                                <?php echo '<span style="cursor:pointer;" onclick="popupWindow2(\'' . tep_href_link('specs.php', tep_get_path($categories['categories_id'])) . '\');">' . tep_image(DIR_WS_ICONS . 'folder_blue.gif', ICON_FOLDER) . '</span>'; ?>
                                                                                <?php echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, tep_get_path($categories['categories_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER) . '</a>&nbsp;<b><a href="' . tep_href_link(FILENAME_CATEGORIES, tep_get_path($categories['categories_id'])) . '">' . $categories['categories_name'] . '</a></b>'; ?>
                                                                            </td>
                                                                            ...
                                                                            <td class="dataTableContent" align="center">
                                                                                <input size="2" type="text"
                                                                                       name="cat_id[<?php echo $categories['categories_id']; ?>]"
                                                                                       value="<?php echo $categories['sort_order']; ?>">
                                                                            </td> <!-- // tth-->
                                                                            <td class="dataTableContent"
...
//$numr=500;
                                                                        ...
                                                                        $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_group,p.products_model_tag, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_to_xml, p.products_sort_order, p.products_model, p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . $stl . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' order by pd.products_name" . " limit " . $page . "," . $max_count);
...
// EOF: KategorienAdmin / OLISWISS
                                                                            ...
                                                                            ?>
                                                                            <td></td>
                                                                            <td class="dataTableContent"><?php if (strstr($admin_right_access, "PDELETE") or $_SESSION['login_id'] = 1) { ?>
                                                                                <input title="<?php echo $products[products_id] . " ddd " . $products[categories_id]; ?>"
                                                                                       name="prod_id[<?php echo $products[products_id]; ?>]"
                                                                                       type="checkbox"
                                                                                       value="<?php echo $products[categories_id]; ?>"/>&nbsp;<?php }
                                                                                echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '">' . $products['products_name']; ?></a>
                                                                            </td>

                                                                            <td class="dataTableContent" align="center">
                                                                                <input type="number" step="1" min="0" name="group[<?=$products['products_id']?>]" value="<?=$products['products_group']?>"></td>
                                                                            <td class="dataTableContent" align="center">
                                                                                <input type="text" size="5" name="model[<?=$products['products_id']?>]" value="<?=$products['products_model_tag']?>"></td>
                                                                            ...
                                                                            <td class="dataTableContent" align="center">
                                                                                <input size="2" type="text"
                                                                                           name="product_id[<?php echo $products['products_id']; ?>]"
                                                                                       value="<?php echo $products['products_sort_order']; ?>">
                                                                            </td>
                                                                            <?php
                                                                            //---by iHolder---
                                                                            if (sizeof($products) > 0) {
                                                                                ...
                                                                                <td class="dataTableContent"
                                                                                    align="center">
                                                                                    <?php
                                                                                    echo '<input size="3" name="qprid[' . $products['products_id'] . ']" value="' . $products['products_quantity'] . '">';
                                                                                    ?>
                                                                                </td>
                                                                            <?php }
                                                                            //---by iHolder---
                                                                            ...
                                                                            <td class="dataTableContent" align="right">
                                                                                ...
                                                                                <?php if ($products['products_id'] == $products_properties_id) {
                                                                                    echo '<a href="javascript:popupPropertiesWindow(\'' . tep_href_link(FILENAME_PRODUCTS_PROPERTIES_POPUP, 'cID=' . $current_category_id . '&pID=' . $products['products_id']) . '\')">' . tep_image(DIR_WS_ICONS . 'icon_properties_change.gif', IMAGE_PROPERTIES_POPUP_ADD_CHANGE_DELETE, 13, 19) . '</a>';
                                                                                } else {
                                                                                    echo '<a href="javascript:popupPropertiesWindow(\'' . tep_href_link(FILENAME_PRODUCTS_PROPERTIES_POPUP, 'cID=' . $current_category_id . '&pID=' . $products['products_id']) . '\')">' . tep_image(DIR_WS_ICONS . 'icon_properties_add.gif', IMAGE_PROPERTIES_POPUP_ADD, 13, 19) . '</a>';
                                                                                } ?>&nbsp;
...
// EOF: KategorienAdmin / OLISWISS
                                                                    ...
                                                                    <tr>
                                                                        <td colspan="6">
                                                                            <table class="table-padding-2">
                                                                                ...
                                                                                <tr>
                                                                                    <td align="right"
                                                                                        class="smallText"><?php if (sizeof($cPath_array) > 0) echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>&nbsp;';
                                                                                        if (!isset($_GET['search'])) echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_category') . '">' . tep_image_button('button_new_category.gif', IMAGE_NEW_CATEGORY) . '</a>&nbsp;<a href="' . tep_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&action=new_product') . '">' . tep_image_button('button_new_product.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>
                                                                                        &nbsp;</td>
                                                                                    <?php
                                                                                    } else {
                                                                                        ?>
                                                                                        <td class="smallText"><?php echo TEXT_TOTAL_PRODUCTS . $numr; ?>
                                                                                            <br>
                                                                                            <?php echo $stp; ?> </td>
                                                                                        <td align="right"
                                                                                            class="smallText"><?php if (sizeof($cPath_array) > 0) echo '<a href="' . tep_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id) . '">' . tep_image_button('button_back.gif', IMAGE_BACK) . '</a>&nbsp;';
                                                                ...
                                                                <input type="hidden" name="cPath"
                                                                       value="<?php echo $_GET['cPath']; ?>">


                                                                <input class="but_cat"
                                                                       name="<?php echo TEXT_BUTTON_DELETE; ?>"
                                                                       value="<?php echo TEXT_BUTTON_DELETE; ?>"
                                                                       type="submit"> <input class="but_cat"
                                                                                             value="<?php echo TEXT_BUTTON_CLEAR; ?>"
                                                                                             name="" type="reset">
                                                                <input class="but_cat2" name="upd"
                                                                       value="<?php echo TEXT_BUTTON_UPDATE; ?>"
                                                                       type="submit">
                                                                <button class="but_cat2" onclick="setGroupOne();return false;">Группа 1</button>
                                                                <input class="but_cat2" type="submit" name="qupd"
                                                                       value="Обновить количество">
                                                                <input class="but_cat2" type="submit" name="gupd"
                                                                       value="Обновить группу">
                                                                <div><input type="number" name="product_order_set" value="1">
                                                                <input class="but_cat2" type="submit" name="supd"
                                                                       value="Установить порядок"></div>
                                                            </form>
...
//<!-- New Attributes Manager End -->
                                                                        ...
                                                                        $contents[] = array('text' => '<br><div class="admin-prod-img">' . tep_info_image($pInfo->products_image, $pInfo->products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</div><br>' . $pInfo->products_image);
