...
if (isset($_POST['search'])) {
    ...
    $products_query = tep_db_query("select p.products_id, p.products_price, p.products_model, pd.products_name from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (p.products_id = pd.products_id) where pd.language_id = '" . $languages_id . "' and (" . $product_search . ") order by pd.products_name limit 60");
    $not_found = ((tep_db_num_rows($products_query)) ? false : true);
}

if (!isset($_POST['search']) && !empty($_POST['add_product_categories_id'])) {
            ...
            <td><table border="0" cellspacing="0" cellpadding="2" style="border: 1px solid #C9C9C9;" align="center">
                    ...
                    <tr class="dataTableRow">
                        <form action="<?php echo tep_href_link(FILENAME_ORDERS_EDIT_ADD_PRODUCT, 'oID=' . $_GET['oID']); ?>" method="POST">
                            <?php
                            /* TODO:Вывод категорий очень сильно тормозит, нужно заменить tep_draw_pull_down_menu
