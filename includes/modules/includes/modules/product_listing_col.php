...
if (!isset($_REQUEST['vls'])){
    if (isset($_SESSION['vls'])){
	$_REQUEST['vls']=$_SESSION['vls'];
	}
    }
if (isset($_REQUEST['vls'])){
    $_SESSION['vls']=$_REQUEST['vls'];
    $LISTING_STYLE=((int)$_REQUEST['vls']==0)?'list':'columns';
        }else{
    $_REQUEST['vls']=($LISTING_STYLE=='list')?0:1;
        }
//---by iHolder make style switchable
...
<table class="table-padding-2">
  <tr><td>
    ...
    <a title="Плитка" class="view_button" href="<?=basename($PHP_SELF).'?'.tep_get_all_get_params(array('vls')).'vls=1';?>"><img src="/images/tiles.png"</a>
    <a title="Строки" class="view_button" href="<?=basename($PHP_SELF).'?'.tep_get_all_get_params(array('vls')).'vls=0';?>"><img src="/images/columns.png"</a>
    </div>
