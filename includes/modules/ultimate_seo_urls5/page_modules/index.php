    ...
    */
    protected $dependencies = array( 'cPath'            => array( 'marker'     => '-c-',
                                     ...
                                     'manufacturers_id' => array( 'marker'     => '-m-',
                                                                  'query'      => "SELECT manufacturers_name FROM :TABLE_MANUFACTURERS_INFO WHERE manufacturers_id=':manufacturers_id' LIMIT 1",
                                                                  'to_replace' => array( ':TABLE_MANUFACTURERS_INFO', ':manufacturers_id' ) ) );
    /**
    ...
    */
    public function buildLink( $page, $parameters, $add_session_id, $connection ) {
      ...
      */
      ...
      if ($this->key=='cPath'){
	$cache=Usu_Main::i()->getVar('categories_cache');
	$last_id=$this->stripPathToLastNumber( $this->keys_index[$this->key]);
	if (isset($cache[$last_id]) && !empty($cache[$last_id])){
	 return $this->returnFinalLink($cache[$last_id]);
//	 return $this->returnFinalLink(urlencode($cache[$last_id]));
	    }
	}
      ...
      switch ( true ) {
        ...
        // This array contains replacements for the to_replace array
          $this->setQuery( array( TABLE_MANUFACTURERS_INFO, (int)$this->keys_index[$this->key] )  );
          break;
        default:
...
/*var_dump($this->returnFinalLink( Usu_Main::i()
