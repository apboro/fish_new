...
define('UNSUBSCRIBE_TEXT','<img src="http://yourfish.ru/unsub.png">');
define('UNSUBSCRIBE_SPAM','<img src="http://yourfish.ru/unsub_spam.png">');
...
//----changed by iHolder substitute unsubscribe in footer---       
	...
	{
	$uid=$mail['customers_uuid'];
	if (strlen(trim($uid))==0){
	//---create uniq ID 
	    do {
		$uid=tep_create_random_value(32);
	        $qry=tep_db_query('select count(*) as cnt from customers where customers_uuid="'.$uid.'"');
		if ($qry!==false){
		    $res=tep_db_fetch_array($qry);
		    }else{$res['cnt']=0;}
	    }while ($res['cnt']>0);
	//---create uniq ID 	    
	    tep_db_query('update customers set customers_uuid="'.$uid.'"
		where customers_id='.$mail['customers_id']);
	    }
/*	$unsubscribe_string='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'">Отписаться от рассылки</a>';
	$unsubscribe_string_spam='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'&agree=1">СПАМ</a>';*/
...
/*------main function to send bulk mail------*/
	if (BULK_USE_BCC){
		    ...
		    tep_mm_sendmail($email_name, $email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $newsletter['subject'], $output_content_html, $output_content_txt);
		    }
...
/*-----uncomment me!!!!*/
if (BULK_USE_BCC){
    tep_mm_sendmail($current_cname, $current_email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $newsletter['subject'], $output_content_html, $output_content_txt,$bcc);
    }
/*-----uncomment me!!!!*/
