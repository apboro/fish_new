...
/**
 * Created by PhpStorm.
 * Author: Efimenko Nikolai
 * Email: powernic1@gmail.com
 * Site:https://github.com/powernic
 * Date: 28.11.2017
 */
 chdir(__DIR__);
...
if(!is_null($exclude_cat)) {
            $query = "SELECT STRAIGHT_JOIN " .
                ...
                p.products_id,pd.products_name, p.products_to_xml, p.products_quantity,p.products_price,p.products_tax_class_id,pc.categories_id,p.manufacturers_id,
    ...
    on p.products_quantity > 0  and p.products_status = '1' AND p.products_price > 0
       and pc.products_id = p.products_id and pc.categories_id NOT IN (" . implode(',', $exclude_cat) . ")
          ...
          p.products_image_xl_6
            FROM " . TABLE_PRODUCTS . " p INNER JOIN " . TABLE_PRODUCTS_TO_CATEGORIES . " AS pc
    on p.products_quantity > 0 and p.products_to_xml ='1' and p.products_status = '1' AND p.products_price > 0
       and pc.products_id = p.products_id and pc.categories_id IN (" . implode(',', $include_cat) . ")
            LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " AS pd ON pd.products_id = p.products_id
            INNER JOIN " . TABLE_CATEGORIES . " AS c ON c.categories_id = pc.categories_id and c.categories_status = '1'
            LEFT JOIN " . TABLE_MANUFACTURERS . " AS m ON m.manufacturers_id = p.manufacturers_id
            left join " . TABLE_SPECIALS . " AS s ON p.products_id = s.products_id
...
//     and pc.products_id = p.products_id and pc.categories_id NOT IN (" . implode(',', $exclude_cat) . ")
            ...
            foreach ($products as $product) {
                $ar = array(167,178,179,180,181,144990,250883,250885,144990,250886,250887,250888,250889,
                    250890);
...
//            'Руси'
        ...
        $ar = array(167,178,179,180,181,144990,250883,250885,250886,250887,250888,250889,
            250890);
        if(in_array($product['products_id'],$ar)) {
            return false;
        }
        ...
        $after = "<currencyId>RUR</currencyId>
            ...
            <dimensions>10/3/5</dimensions>
            {$weight}
            {$vendor}
            {$vendorCode}
            {$description}
            {$params}
            <sales_notes>Самовывоз, курьер. Трансп комп - предоплата</sales_notes>
        </offer>
        ";
...
if (strripos($_SERVER['REQUEST_URI'], '.zip') == false) {
    $filename = 'yandex_market.gz';
} else {
    $filename = 'yandex_market.zip';
}
...
if (!read_cache($cache_output, 'yandex_market.zip', 6 * 60)) {
...
//   YandexMarket::showPromos();
    ...
    file_put_contents(DIR_FS_CACHE . 'yandex_market_tmp.xml', $cache_output);
    shell_exec('cd ' . DIR_FS_CACHE . ' && rm -f yandex_market.zip && zip yandex_market.zip yandex_market_tmp.xml');
    shell_exec('cd ' . DIR_FS_CACHE . ' && rm -f yandex_market.gz && gzip -c yandex_market_tmp.xml > yandex_market.gz');
