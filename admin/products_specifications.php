...
/* unlink by iHolder*/
        if ($link_subcats == 'True' && tep_has_category_subcategories ($categories_id)) {
          $categories_array = array();
          tep_get_subcategories ($categories_array, $categories_id);
if (is_array($categories_array)&&(sizeof($categories_array)>0)){
            $links_query_raw = "select specification_group_id as sgid,categories_id as catid
                                from " . TABLE_SPECIFICATIONS_TO_CATEGORIES . "
                                where specification_group_id = '" . $specification_group_id . "' 
                                  and categories_id in(" . implode(',',$categories_array) . ")
                              ";
	    $links_query = tep_db_query ($links_query_raw);
            if (tep_db_num_rows ($links_query) >0) {	    
		while ($res=tep_db_fetch_array($links_query)){

        	tep_db_query ("delete from " . TABLE_SPECIFICATIONS_TO_CATEGORIES . " 
                         where specification_group_id = '" . $res['sgid'] . "'
                           and categories_id = '" . $res['catid'] . "'
                       ");
                }//--while
	    }//---if any category
