...
?>
<div class="contentContainer" id="address_book_process">
	<h1><?php if (isset($HTTP_GET_VARS['edit'])) { echo HEADING_TITLE_MODIFY_ENTRY; } elseif (isset($HTTP_GET_VARS['delete'])) { echo HEADING_TITLE_DELETE_ENTRY; } else { echo 
		HEADING_TITLE_ADD_ENTRY; } ?>
    </h1>
    <div class="review_box">
    	<div class="content_box">
            ...
            <?php
                if (isset($HTTP_GET_VARS['delete'])) {
            ?>
            <span class="title"><?php echo DELETE_ADDRESS_TITLE; ?></span>
            <p>
            ...
            <span class="link_button">
                <?php echo tep_draw_button(IMAGE_BUTTON_DELETE, 'trash', tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS, 'delete=' . $HTTP_GET_VARS['delete'] . 
                                    '&action=deleteconfirm&formid=' . md5($sessiontoken), 'SSL'), 'primary'); ?>
            </span>
            <span class="link_button">
            ...
            ?>
            <?php echo tep_draw_form('addressbook', tep_href_link(FILENAME_ADDRESS_BOOK_PROCESS, (isset($HTTP_GET_VARS['edit']) ? 'edit=' . 
                    $HTTP_GET_VARS['edit'] : ''), 'SSL'), 'post', 'onsubmit="return check_form(addressbook);"', true); ?>
            
            <?php include(DIR_WS_MODULES . 'address_book_details.php'); ?>
            <?php
                    if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
            ?>
            <?php echo tep_draw_hidden_field('action', 'update') . tep_draw_hidden_field('edit', $HTTP_GET_VARS['edit']) . 
                    tep_draw_button(IMAGE_BUTTON_UPDATE, 'refresh', null, 'primary'); ?>
            <span class="link_button"><?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'triangle-1-w', tep_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL')); ?></span>
            <?php
                } else {
                    if (sizeof($navigation->snapshot) > 0) {
                            $back_link = tep_href_link($navigation->snapshot['page'], tep_array_to_string($navigation->snapshot['get'], array(tep_session_name())), 
            ...
            ?>
            <div class="buttonSet">
                <span class="buttonAction"><?php echo tep_draw_hidden_field('action', 'process') . tep_draw_button(IMAGE_BUTTON_CONTINUE, 'triangle-1-e', null, 'primary'); ?>
                </span>
    
                <span class="link_button"><?php echo tep_draw_button(IMAGE_BUTTON_BACK, 'triangle-1-w', $back_link); ?></span>
            </div>
