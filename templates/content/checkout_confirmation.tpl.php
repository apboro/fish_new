...
?>
            <td width="<?php echo (($sendto != false) ? '70%' : '100%'); ?>" valign="top"><table class="table-padding-0">
              <tr>
...
//  echo ' мистер путин ';
 // echo $GLOBALS[$class];
 /* echo $GLOBALS[$class]->output[0]['text'];
  echo $GLOBALS[$class]->output[0]['value'];
  echo $GLOBALS[$class]->output[1]['value'];
//    $order_total_modules->process();
      //tep_db_query("UPDATE `orders_total` SET `value` = 500 ` where orders_id` = 56497" );
  echo $order_total_array[1]['value'];
  echo $order_totals[1]['value'];
  echo $order_totals[0]['value'];
	 echo $order_total_modules[0]['value'];
echo $order_total[0]['value'];	 
echo 'lhebl';*/
	  //$order_total_modules[1]['value'] = 450;
...
// BOF: Lango modified for print order mod
  if (is_array($payment_modules->modules)) {
    ...
    if ($confirmation = $payment_modules->confirmation()) {
      ...
      if (!isset($confirmation['display'])){$payment_title=$payment_info;}
        else{$payment_title = $confirmation['display'];}
        ...
        <td><table border="0" width="100%" cellspacing="1" cellpadding="2" class="infoBox">
          <tr class="infoBoxContents">
            <td><table border="0" cellspacing="0" cellpadding="2">
              <tr>
                <td class="main" colspan="4"><?php echo $confirmation['display']; ?></td>
              </tr>
<?php
...
?>
      ...
      <tr>
        <td><table class="table-padding-0">
          <tr>
            <td width="25%"><table class="table-padding-0">
              <tr>
