...
class YandexMarket
{
     ...
     */
    public static function showOffers($currencies, $exclude_cat = array(), $limit = 0)
    {
        echo "<offers>";
        ...
        if (!read_cache($offers, 'offers_yml.cache', self::$cache_time)) {
            $query = "SELECT STRAIGHT_JOIN " .
                ...
                p.products_id,pd.products_name,p.products_quantity,p.products_price,p.products_tax_class_id,pc.categories_id,p.manufacturers_id,
    ...
    on p.products_quantity > 0 and p.products_status = '1' AND p.products_price > 0
       and pc.products_id = p.products_id and pc.categories_id NOT IN (" . implode(',', $exclude_cat) . ")
            ...
            foreach ($products as $product) {
                self::getOffer($product, $currencies, $clothes_cat, $param_names);
            }
    ...
    /**Рендер Промокодов для Прайс-листа
     */
    ...
    private function updatePrices($currencies, $exclude_cat = array())
    {
        $query = "SELECT p.*,ym.before, ym.after,ym.cat_id as categories_id
            ...
            ym.cat_id NOT IN (" . implode(',', $exclude_cat) . ") and p.products_id=59879";
        $products_query = tep_db_query($query);
        while ($product = tep_db_fetch_array($products_query)) {
    ...
    private function getOffer($product, $currencies, $clothes_cat, $param_names)
    {
        ...
        $after = "<currencyId>RUR</currencyId>
            ...
            {$description}
            {$params}
            <sales_notes>Скидки до 15%.{$min_quantity}</sales_notes>
        </offer>
        ";
...
*/
if (true || !read_cache($cache_output, 'yandex_market.zip', 6 * 60)) {
    ...
    YandexMarket::showOffers($currencies, $exclude_cat);
