...
$attr_count=0;
if (is_array($attrs)){
    foreach($attrs as $attrib=>$attrib_value){
	if ($attrib_value==-1){continue;}
	$attr_count++;
	$value_qry=tep_db_query('SELECT products_options_values_name as povn FROM products_options_values where products_options_values_id='.$attrib_value);
	$value='';	
	if ($value_qry!==false){
	    $value_res=tep_db_fetch_array($value_qry);
	    $value=$value_res['povn'];
	    }
	if (strlen($value)>0){
	    $qry_values=tep_db_query('select pov.products_options_values_id as povid from `products_options_values` pov left join
    		 `products_options_values_to_products_options` povtpo on
    		 (pov.products_options_values_id=povtpo.products_options_values_id)
		where pov.products_options_values_name="'.tep_db_input($value).'"
		and povtpo.products_options_id='.$attrib);
		if ($qry_values!==false){
	    	    while($qry_values_res=tep_db_fetch_array($qry_values)){
			$attr_values[]=$qry_values_res['povid'];		    
			}
		    }
	    }
//	$attr_values[]=$attrib_value;
//	echo $attrib.'--'.$attrib_value.'<br>';
	}
    if (sizeof($attr_values)>0){
    $pids_query='SELECT products_id,count(*) as cnt FROM products_attributes where options_values_id in ('.
    implode($attr_values,',').') group by products_id having cnt='.$attr_count;
//    echo $pids_query;
    $pids_qry=tep_db_query($pids_query);
    if ($pids_qry!==false){
	while ($pids_res=tep_db_fetch_array($pids_qry)){
	    $pids[]=$pids_res['products_id'];
	    }
	}
...
<table><tr><td style="border:1px dashed black;background-color:#ddd">
<?php/*
//if (sizeof($pids)>0){
    echo '<p style="margin:0;padding:10px;background-color:pink;font-weight:bold;font-size:16px;border:1px solid black;"> Найдено: '.sizeof($pids).' товаров</p>';
//    }
?>
<form action="<?php echo $_SERVER['SCRIPT_NAME']?>" method="POST" name="search_attr" id="sat">
<?php 
$attr_qry='select distinct * from products_options where products_options_id is not null order by products_options_name';
$aqry=tep_db_query($attr_qry);
if ($aqry!==false){
echo '<table border="1">';
echo '<tr style="text-align:center;font-size:16px;color:black;font-weight:bold;background-color:#bbb;"><td>Аттрибут</td><td>Значение</td></tr>';    
    while($rq=tep_db_fetch_array($aqry)){
	echo '<tr><td>'.$rq['products_options_name'].'</td><td><select name="attr['.$rq['products_options_id'].']">';
	echo '<option ';
	if ((isset($attrs[$rq['products_options_id']]))&&(($attrs[$rq['products_options_id']])==-1)){
	    echo selected;
	    }
	echo ' value="-1">Не определено</option>';
// get values for each attributes
	$v_qry='SELECT distinct 
	    povtpo.products_options_values_id,
	    povtpo.products_options_id,
	    pov.products_options_values_name
	FROM `products_options_values_to_products_options` povtpo
	inner join products_options po on (povtpo.products_options_id=po.products_options_id)
	inner join  products_options_values pov on (pov.products_options_values_id=povtpo.products_options_values_id)
	inner join products_attributes pa on (pa.options_values_id=pov.products_options_values_id)
	where povtpo.products_options_id='.$rq['products_options_id'].
	' and length(products_options_values_name)>0 '.
	' order by products_options_values_name';

	$vqry=tep_db_query($v_qry);
	if ($vqry!==false){
	    while ($vres=tep_db_fetch_array($vqry)){
		echo '<option value="'.$vres['products_options_values_id'].'" ';
	if ((isset($attrs[$rq['products_options_id']]))&&(($attrs[$rq['products_options_id']])==$vres['products_options_values_id'])){
	    echo selected;
	    }
		echo '>'.$vres['products_options_values_name'].'</option>';
		};
	    }
// get values for each attributes
	echo '</select></td></tr>';
	}
echo '<tr><td colspan="2" style="border:1px solid black;padding:10px;background-color:pink;text-align:center"><input type="submit" value="Посмотреть"></td></tr>';
echo '</table>';
    }
*/?>
...
if (is_array($pids)&&(sizeof($pids)>0)){
      $products_query = tep_db_query("select p.products_tax_class_id, p.products_id, pd.products_name, p.products_quantity, p.products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.products_model, p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_id = p2c.products_id and pd.products_id in(" .implode($pids,',') . ") order by pd.products_name"); 
    }else{
