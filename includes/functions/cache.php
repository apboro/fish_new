...
function tep_cache_related_products($auto_expire = false, $refresh = false) {
    global $HTTP_GET_VARS, $language, $languages_id;

    $cache_output = '';

    if (isset($HTTP_GET_VARS['products_id']) && is_numeric($HTTP_GET_VARS['products_id'])) {
        if (($refresh == true) || !read_cache($cache_output, 'related_products-' . $language . '.cache' . $HTTP_GET_VARS['products_id'], $auto_expire)) {
            ob_start();
            include(DIR_WS_MODULES . FILENAME_CONTENT_RELATED_PRODUCTS);
            $cache_output = ob_get_contents();
            ob_end_clean();
            write_cache($cache_output, 'related_products-' . $language . '.cache' . $HTTP_GET_VARS['products_id']);
        }
    }
