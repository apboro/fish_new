  ...
  if (tep_not_null($set)) {
    switch ($set) {
      ...
      case 'ordertotal':
        ...
        break;
/*add osCommerce boxes*/
      case 'mboxes':
        $module_type = 'boxes';
        $module_directory = DIR_FS_CATALOG.'m/'.DIR_WS_MODULES. 'boxes/';
        $module_key = 'MODULE_BOXES_INSTALLED';
        define('HEADING_TITLE', 'Мобильные боксы');
        break;
/*add osCommerce boxes*/
      case 'payment':
      default:
...
<?php
  ...
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
    ...
    /*---mobile boxes---*/
    if ($set=='mboxes'){
    include(DIR_FS_CATALOG.'m/'.DIR_WS_LANGUAGES . $language . '/modules/' . $module_type . '/' . $file);
...
/*    echo $check['configuration_value'].'<br>';
    echo implode(';', $installed_modules);*/
