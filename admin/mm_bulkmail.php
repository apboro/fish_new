<?php
define('BULK_EMAIL_COUNT',300);
define('BULK_USE_BCC', false);
  require('includes/application_top.php');
define('UNSUBSCRIBE_TEXT','<img src="'.echoHTTP().'yourfish.ru/unsub.png">');
define('UNSUBSCRIBE_SPAM','<img src="'.echoHTTP().'yourfish.ru/unsub_spam.png">');
      ...
      <tr><?php echo tep_draw_form('newsletter', FILENAME_MM_BULKMAIL, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=' . $form_action); if ($form_action == 'update') echo tep_draw_hidden_field('newsletter_id', $nID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td class="main"><?php echo tep_draw_input_field('mailrate', $nInfo->mailrate, 'size="4" maxlength="4"', '').NOTES_MAILRATE; ?></td>
          </tr>
...
<?php
    ...
    //tep_mm_sendmail(STORE_OWNER, "Bf3310@yandex.ru", STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $newsletter['subject'], $output_content_html, $output_content_txt);
      $unsubscribe_string='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'">'.UNSUBSCRIBE_TEXT.'</a>';
      $unsubscribe_string_spam='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'&agree=1">'.UNSUBSCRIBE_SPAM.'</a>';

      $output_content_html=str_replace('{unsubscribe}',$unsubscribe_string,$output_content_html);
      $output_content_html=str_replace('[unsubscribe]',$unsubscribe_string_spam,$output_content_html);
...
////////////////////////////////////////////////SEND      SEND            SEND  
 ...
 require(DIR_WS_MODULES . 'mail_manager/' . $newsletter['module'] . substr($PHP_SELF, strrpos($PHP_SELF, '.')));
    ...
    //start mailing loop
       if($mail['mmstatus']=='0'){
       if (BULK_USE_BCC){
        $current_cname=STORE_OWNER;
        $current_email=STORE_OWNER_EMAIL_ADDRESS;
        $bcc=array();
        }
      $log_data='';
...
//var_dump($mail);exit;
       for ($i=0;$i<BULK_EMAIL_COUNT;$i++){
	    if (is_array($mail)&&($mail['mmstatus']==0)){
/*------main function to send bulk mail------*/
      $email=$mail['customers_email_address'];
      $email_name = $mail['customers_firstname'] . ' ' . $mail['customers_lastname'];
      $email_cid=$mail['customers_id'];
	/*--verify of status not changed---*/
//        $qr=tep_db_query("select mmstatus from ".TABLE_CUSTOMERS." WHERE   customers_email_address ='" . $email . "'  Limit 1");
        $qr=tep_db_query("select mmstatus from ".TABLE_CUSTOMERS." WHERE   customers_id ='" . $email_cid . "'  Limit 1");
	$rr=tep_db_fetch_array($qr);
//    var_dump($rr);exit;
	if ($rr['mmstatus']<>0){continue;}
	/*--verify of status not changed---*/

//----changed by iHolder substitute unsubscribe in footer---       
$mail_output_content_html=$output_content_html;
    if (
	((strpos($output_content_html,'{unsubscribe}')>0)||
	(strpos($output_content_html,'[unsubscribe]')>0))
	&&
	(strlen($mail['customers_id'])>0)
	)
	{

	     $uid=GetCustomersUUID($email_cid);

	$unsubscribe_string='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'">'.UNSUBSCRIBE_TEXT.'</a>';
	$unsubscribe_string_spam='<a href="'.HTTP_SERVER.'/unsubscribe.php?uid='.$uid.'&agree=1">'.UNSUBSCRIBE_SPAM.'</a>';

	$mail_output_content_html=str_replace('{unsubscribe}',$unsubscribe_string,$output_content_html);
	$mail_output_content_html=str_replace('[unsubscribe]',$unsubscribe_string_spam,$mail_output_content_html);
    $mail_output_content_html=str_replace('{username}',$email_name,$mail_output_content_html);
	}
//----changed by iHolder substitute unsubscribe in footer---
      ...
      tep_db_query("UPDATE ".TABLE_CUSTOMERS." SET mmstatus =  '9'  WHERE   customers_id ='" . $email_cid . "'  Limit 1");
      $log_data .= ' <tr>
				  <td>' . $email_name . '</td>
				  <td>' . $email . '</td>
				  <td>' . $date_time . '</td>
...
/*------main function to send bulk mail------*/
            $t = explode(" ",microtime());
            $test = date("m-d-y H:i:s",$t[1]).substr((string)$t[0],1,4);

	if (BULK_USE_BCC){
		    $bcc[]=array('name'=>$email_name,'address'=>$email); }
		    else{
		    tep_mm_sendmail($email_name, $email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $newsletter['subject'], $mail_output_content_html, $output_content_txt);
		    }
...
/*-----uncomment me!!!!*/
if (BULK_USE_BCC){
    tep_mm_sendmail($current_cname, $current_email, STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $newsletter['subject'], $mail_output_content_html, $output_content_txt,$bcc);
    }
...
//        echo "<meta http-equiv='refresh' content=$mailrate>";
    ...
    // end loop
	  ...
	  $log_email_body = 'The bulkmail sending of ' . $newsletter['title'] . ' complete.  You can view the log file containing information on who was emailed and more information <a target="_blank" href="' . HTTP_CATALOG_SERVER . DIR_WS_ADMIN . $log_file_location . '">here</a>.';
...
?>
              <tr>
                <td colspan="6"><table class="table-padding-2">
                  <tr>
                    <td class="smallText top-valign"><?php echo $newsletters_split->display_count($newsletters_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $HTTP_GET_VARS['page'], TEXT_DISPLAY_NUMBER_OF_NEWSLETTERS); ?></td>
