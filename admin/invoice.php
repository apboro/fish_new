...
Москва, 4й Лихачевский переулок дом 2 строение 2<br> 
<br>
<hr>
...
<table border="0" width="100%" id="table1" cellspacing="0">
	...
	<tr>
		<td width="3%" style="border-style: solid; border-width: 1px" align="center"><b><?php echo TEXT_15; ?></b></td>
		<td width="67%" colspan="2" style="border-style: solid; border-width: 1px" align="center"><?php echo TEXT_14; ?></td>
		
		<td width="10%" style="border-style: solid; border-width: 1px" align="center"><b><?php echo TEXT_13; ?></b></td>
...
<?php
    ...
    $qty_total+= $order->products[$i]['qty'];
    $num = $i +1;
      echo '      <tr>' . "\n" .
          '        <td width="3%" style="border-style: solid; border-width: 1px">' . $order->products[$i]['qty'] . '</td>' . "\n" .
             '        <td width="75%" colspan="2" style="border-style: solid; border-width: 1px; font-size:.8em">' . $order->products[$i]['name']; 
    ...
    echo  '        </td><td width="10%" style="border-style: solid; border-width: 1px; font-size:.8em">' . $order->products[$i]['model'] ;  
      echo '        </td>' . "\n" .
...
?>
  ...
  for ($i = 0, $n = sizeof($order->totals); $i < $n; $i++) {
         if ( $order->totals[$i]['title']=='Стоимость товара:'){
	        echo '          <tr style = "font-size:.9em">' . "\n" .
	    '<td style="border-style: solid; border-width: 1px; font-size:0.9em">'.$qty_total.'</td>'.
	 '<td style="width:15%;padding-left:5px;border:1px solid black;border-right:none;">Количество</td>'.
	 '<td style="width:30%;border:1px solid black;border-left:1px solid black;border-right:none;"></td>'.
        '<td colspan="2" style="border-style: solid; border-width: 1px; border-left:none;font-size:0.9em">
        
         <p align="right"><b>' . $order->totals[$i]['title'] . '</b></p></td>' . "\n" ;
         echo        '<td width="8%" style="border-style: solid; border-width: 1px; font-size:1.0em">' . number_format(tep_round($order->totals[$i]['value']*$order->info['currency_value'], $currencies->currencies[$order->info['currency']]['decimal_places']), 0) . ' руб.</td>' . "\n" .
         '          </tr>' . "\n";

            }else {
                echo '          <tr style = "font-size:.9em">' . "\n" .
         '            <td colspan="5" style="border-style: solid; border-width: 1px; font-size:0.9em"><p align="right"><b>' . $order->totals[$i]['title'] . '</b></p></td>' . "\n" ;
         echo        '<td width="8%" style="border-style: solid; border-width: 1px; font-size:1.0em">' . number_format(tep_round($order->totals[$i]['value']*$order->info['currency_value'], $currencies->currencies[$order->info['currency']]['decimal_places']), 0) . ' руб.</td>' . "\n" .
         '          </tr>' . "\n";
...
<p>
<b><?php echo TEXT_18; ?> <?php echo $iw->get($total_summ['value']); ?><?php echo TEXT_24; ?></b></p>
<p>&nbsp;</p></font></u>
<p><b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
