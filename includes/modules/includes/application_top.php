...
\*--------------------------------------------------------*/
$cache_pages = array('discount.php','index.php', 'product_info.php', 'information.php', 'price.php', 'products_new.php', 'featured_products.php');
...
switch($_GET['cPath']){
	...
	if (sizeof($_GET)>0){
		...
		if (preg_match('|([\d]+)|',$key,$arr)){
		    tep_redirect(tep_href_link(FILENAME_PRODUCT_INFO,'products_id='.(int)$arr[1]));
		    }
		};
	    }
	tep_redirect(tep_href_link(FILENAME_DEFAULT));
...
global $_GET;
if (isset($_GET['cPath'])){
    $latest=end(explode('_',$_GET['cPath']));
    $rcPath='';$ct=array();
