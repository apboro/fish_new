...
<?php
  ...
  $sql = "SELECT MAX(p.products_price) max_price, MIN(p.products_price) min_price
            FROM products p 
            INNER JOIN products_to_categories p2c ON p2c.products_id = p.products_id AND p2c.categories_id = ".$browse_category_id."
            WHERE p.products_status = '1'";


     $res = tep_db_query($sql);
     $price_limit = tep_db_fetch_array($res);
     $min = explode(",", $price_limit['min_price']);
     $max = explode(",", $price_limit['max_price']);
     if ($price_limit['min_price'] > 0) {
     $result = "
     ". round($min[0]) ." - ". round($max[0])." руб.
     ";
     }
  ...
  // BOF Enable - Disable Categories Contribution--------------------------------------
    ...
    while ($current_categories = tep_db_fetch_array($arr_current_categories_query)) {
      ...
      if (BRWCAT_ICON_MODE == 'text') {
        ...
        $content_box_contents[$row][$col] = array('align'  => 'center',
                                                  'params' => 'class="sukavramke" valign="center"',
                                                  'text'   => '<div class="tablodiv"><div class="ramka" align="center"><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '">' .
                                                              tep_image(DIR_WS_IMAGES . $current_categories['categories_image'],
                                                                        $current_category_name,
                                                                        SUBCATEGORY_IMAGE_WIDTH, SUBCATEGORY_IMAGE_HEIGHT) .
                                                              '</a><div align="center"><br><a href="' . tep_href_link(FILENAME_DEFAULT, $cPath_cur, 'NONSSL') . '"><b>' .
                                                              $current_category_name . '</b></a>
                                                              <br><br><br><br> ' . get_price_range($current_categories['categories_id'] ) . '</div></div></div> ');
