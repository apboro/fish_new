    ...
    <script language="javascript" type="text/javascript"><!--
        ...
        function setShipping(method) {
            if (document.getElementById("ot_shipping[title]")) {
                if ('cdek_cdek' == document.getElementById("shipping_radio_" + method).value) {
                    var in_first = false;
                    if (jQuery('#cdek_block').length == 0) {
                        jQuery(document.getElementById("ot_shipping[title]"))
                            .parent()
                            .append('<div id="cdek_block"></div>');
                        in_first = true;
                    }
                    shipping_block = jQuery('#cdek_block');
                    jQuery('#cdek_block').show();
                    if(in_first) {
                        var shipping_html = '<input  name="city" autocomplete="off" type="text" placeholder="ВВЕДИТЕ ВАШ ГОРОД"><br>' +
                            '<input type="hidden" name="cityID" >' +
                                '<input type="hidden" name="cdek_tarif" >'+
                            '<a href="#yandmap" id="showmap">Выбор пункта на карте</a><br><span id="cdek_postbox">' +
                            '<select id="postbox" name="cdek_postbox" >' +
                            '</select><br>' +
                            '</span>' +
                            '<span id="cdek_way"></span>';
                        shipping_block.html(shipping_html);
                        if (window.selectPlace === undefined) {
                            window.selectPlace = function ($this, addr, code) {
                                jQuery('#postbox').val(code);
                                jQuery.fancybox.close();
                                return false;
                            };
                        }
                        jQuery("#postbox").change(function(){
                            var place = jQuery('#postbox option:selected').text();
                            document.getElementById("ot_shipping[title]").value = "Пункт выдачи СДЭК (" + jQuery("input[name='city']").val()+', '+ place + "):";
                        });
                        ...
                        jQuery("input[name='city']").autocomplete({
                            source: function (request, response) {
                                jQuery.ajax({
                                    ...
                                    dataType: "jsonp",
                                    crossDomain: true,
                                    appendTo: "input[name='city']",
                                    data: {
                                        q: function () {
                                            return jQuery("input[name='city']").val()
                                        },
                            ...
                            select: function (event, ui) {
                                if (jQuery("input[name='cityID']").length > 0) {
                                    jQuery("input[name='city']").val(ui.item.value.substr(0, 30));
                                    ...
                                    jQuery.ajax({
                                        ...
                                        success: function (data) {
                                            if (data.error || !('addressList' in data)) {
                                                ...
                                                jQuery(document.getElementById("ot_shipping[value]")).val(data.price);
                                                ...
                                                if (data.addressList.length > 0) {
                                                    ...
                                                    jQuery.each(data.addressList, function (ind) {
                                                        ...
                                                        var myPlacemark = new ymaps.Placemark(coord, {
                                                            ...
                                                            balloonContentFooter: item.WorkTime + '<br><button onclick="selectPlace(this,\'' + item.Address + '\',\'' + item.Code + '\')">Выбрать</button>',
                                                        });
                                                ...
                                                jQuery("input[name='cdek_tarif']").val(data.tariffId);
                                                var place = jQuery('#postbox option:selected').text();
                                                document.getElementById("ot_shipping[title]").value = "Пункт выдачи СДЭК (" + jQuery("input[name='city']").val()+', '+ place + "):";
                                            }
                                        }
