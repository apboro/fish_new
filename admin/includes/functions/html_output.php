...
// Output a form
  ...
  function tep_draw_input_field2($name, $value = '', $parameters = '', $required = false, $type = 'text', $reinsert_value = true) {
    $field = '<input type="' . tep_output_string($type) . '" name="' . tep_output_string($name) . '"size="75"';
...
// Output a form textarea field
  ...
  // Output a form textarea field for my mommy
  function tep_draw_textarea_field2($name, $wrap, $width, $height, $text = '', $parameters = '', $reinsert_value = true) {
    $field = '<textarea style ="float:left;" name="' . tep_output_string($name) . '" wrap="' . tep_output_string($wrap) . '" cols="' . tep_output_string($width) . '" rows="' . tep_output_string($height) . '"';
...
// Hide form elements
 	  ...
 	  function tep_draw_select2_menu($name, $values, $default = '', $parameters = '', $required = false)
      {
          $html = '<select style="width:300px" class="select2-menu" ';
          if (tep_not_null($parameters)) $html .= ' ' . $parameters;
          $html .= ' name="'.tep_output_string($name) . '"  ><option disabled>Выберите категорию</option></select>';
          return $html;
      }
...
//function tep_draw_pull_down_menu($name, $values, $default = '', $parameters = '', $required = false) {
  ...
  function tep_draw_pull_down_menu($name, $values, $default = '', $parameters = '', $required = false)
  {
      if (in_array($name,array('cPath','categories_id','move_to_category_id','link_category_id','epcust_category_filter'))) {
          return tep_draw_select2_menu($name, $values, $default, $parameters, $required);
      }
